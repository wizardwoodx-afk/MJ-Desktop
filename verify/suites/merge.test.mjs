  ...
  "src/vouch/ipc/client.ts"() {
    ...
    ipc2 = {
      async secretGet(secretRef) {
        return await invoke3("secret_get", { secretRef });
      },
      async secretSet(secretRef, value) {
        return await invoke3("secret_set", { secretRef, value });
      },
      async notifyApproval(title, body) {
        await invoke3("notify_approval", { title, body });
      },
      async appInfo() {
        return await invoke3("app_info");
      }
    };
  }
});
...
var STORAGE_KEY2 = "mj.issuerkey.v1";
...
function hostRunnerDeps(opts) {
  ...
  return {
    ...
    cliInvoke: async (req) => {
      if (isNative) {
        try {
          const r = await ipc.cliInvoke(req.bin, req.argv.join(" "), req.cwd, req.timeoutSecs, req.argv);
...
describe("merge \u2014 one mission ID, one chain, one state (16.0)", () => {
  it("the merged chain is tamper-evident: flip 1 byte \u2192 chain broken", { timeout: 2e4 }, async () => {
    ...
    try {
      ...
      const clean = await verifyProofReceipt2(rc);
  ...
  it("the shell join: the face door renders the Vouch page, labeled Vouch", () => {
    const app = read("src/App.tsx");
    const nav = read("src/app/nav.ts");
    assert.ok(app.includes('import("./vouch/pages/VouchPage")'), "the shell routes the face door to the Vouch control plane");
    assert.ok(app.includes('{page === "teammate" && <ErrorBoundary label="vouch"><VouchPage /></ErrorBoundary>}'), "the door renders VouchPage");
    assert.ok(/key: "teammate", label: "Vouch"/.test(nav), "the public label is Vouch");
    ...
    assert.equal(VH_TITLE, `Vouch Harbor ${VH_SHORT}`, "the product title is Vouch Harbor");
    assert.ok(/<title>Vouch Harbor<\/title>/.test(read("index.html")), "the window title is Vouch Harbor");
    assert.equal(VH_VERSION, "16.9.0", "one product version line: the single VH_VERSION stamps everything");
  });
});
