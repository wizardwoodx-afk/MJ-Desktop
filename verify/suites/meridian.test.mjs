...
var NODE_METHODS = {
  ...
  "agent.coder": {
    method: "CODE (built in): write the smallest diff that passes the tests. No drive-by refactors, no new dependencies without a stated reason."
  },
  "agent.debugger": {
    method: "DEBUG (built in): reproduce \u2192 isolate \u2192 hypothesise \u2192 fix \u2192 prove with a re-run of the failing case. The bug is only closed by the re-run."
  },
  "agent.tester": {
    method: "TEST (built in): derive tests from the acceptance criteria, edge cases before happy path, and report exact coverage of what was claimed."
  },
  "agent.critic": {
    method: "CRITIQUE (built in): attack the plan, not the author. Every objection names the step it breaks and what evidence would change its mind."
  },
  "agent.reviewer": {
    method: "REVIEW (built in): diff-only review \u2014 comment on what changed, block on correctness, style nits last and clearly labelled as nits."
  },
  "agent.qa": {
    method: "QA (built in): turn the success criteria into an acceptance checklist and check every line \u2014 a pass is only a pass with evidence attached."
  },
  "agent.docs": {
    method: "DOCUMENT (built in): write docs from the diff, not from aspiration \u2014 what changed, why, and how to migrate. A changelog entry always."
  },
  "agent.security": {
    method: "SECURE (built in): threat-model the data paths through this node, list each threat with severity, and confirm secrets are never logged or echoed."
  },
  "agent.evolution": {
    method: "EVOLVE (built in): run the current mission, measure against the rating signal, and propose exactly one change with the evidence for it."
  },
  "agent.custom": {
    method: "CUSTOM (built in): follow the procedures exactly as written \u2014 the procedures ARE the method. No improvisation, no extra steps."
  },
  "cap.http": {
    method: "CALL (built in): one request per run, timeout enforced, status and body size checked before anything downstream sees them."
  },
  "cap.filesystem": {
    method: "FILE (built in): one operation per run, resolved paths only \u2014 no traversal outside the workspace root."
  },
  "cap.terminal": {
    method: "RUN (built in): execute once with the configured timeout, capture stdout/stderr separately, and fail loudly on non-zero exit."
  },
  "cap.browser": {
    method: "DRIVE (built in): attach to a real browser session, act one step at a time, verify between steps."
  },
  "cap.json": {
...
function composeAssignment(def, config = {}) {
...
var root = ".";
var read = (p2) => fs.readFileSync(path.join(root, p2), "utf8");
var icons = read("src/canvas/icons.tsx");
var canvas = read("src/canvas/Canvas.tsx");
var overlay = read("src/panels/ShortcutsOverlay.tsx");
var inspector = read("src/panels/Inspector.tsx");
var storeSrc = read("src/graph/store.ts");
section("1. the FOUNDRY icon grammar \u2014 1.9 engraved strokes, brass facet chips");
var caseCount = (icons.match(/case "/g) ?? []).length;
var facetCount = (icons.match(/facet\(/g) ?? []).length;
...
var DEFINITIONS = [...DEFINITIONS_BY_ID.values()];
var agentDefs = DEFINITIONS.filter((d) => d.category === "agent");
var CORE_AGENTS = ["agent.planner", "agent.researcher", "agent.browser", "agent.coder", "agent.debugger", "agent.tester", "agent.critic", "agent.reviewer", "agent.qa", "agent.docs", "agent.security"];
ok(
  `the core agents carry in-built methods (${CORE_AGENTS.length} of ${agentDefs.length} agent defs)`,
  CORE_AGENTS.every((id) => typeof NODE_METHODS[id]?.method === "string"),
  CORE_AGENTS.filter((id) => !NODE_METHODS[id]).join(", ")
);
ok(
  `no agent def is left without an honest method (${agentDefs.length} defs, role packs fall back to the category verb)`,
...
ok(
  "capability nodes carry methods too (webhook/cron/vector)",
  ["cap.webhook", "cap.cron", "cap.vector"].every((id) => NODE_METHODS[id]?.method),
  "missing cap method"
);
ok(
...
ok(
  "methodFor's last resort admits ignorance",
  methodFor({ id: "x.y", category: "misc" }) === "Method not yet specified",
  "fallback lies"
);
var foldKeys = Object.entries(NODE_FIELDS).filter(([, v]) => v.fold === "pre-existing").map(([k]) => k);
ok(`fold-fields are labelled as pre-existing (${foldKeys.length} keys)`, foldKeys.length >= 15, foldKeys.join(", "));
ok(
  "genuinely-new keys carry a one-line default hint",
  Object.entries(NODE_FIELDS).filter(([, v]) => !v.fold).every(([, v]) => (v.def ?? "").length > 0),
  "a new key has no hint"
);
{
...
ok("no asst-* styling survives", !/\.asst-/.test(read("src/styles/vouch.css")), "asst css remains");
ok("the Ctrl+J chord is gone from the overlay", !read("src/panels/ShortcutsOverlay.tsx").includes("Ctrl+J"), "chord still listed");
ok(
  "custom-node drafting survives outside Assist (HomePage field)",
  read("src/pages/HomePage.tsx").includes("addCustom") && /customNode/.test(read("src/domain/workflowBuilder.ts")),
  "custom-node path lost"
);
...
ok(
  "the state lamp breathes on run and goes quiet under reduced motion",
  /\.glyph-light\.on \{[^}]*animation: rd-lamp/.test(read("src/styles/redesign.css")),
  "lamp pulse lost"
);
...
var skills = (rel) => {
  try {
    return fs.readFileSync(path.join(root, "skills", rel), "utf8");
  } catch {
    return "";
  }
};
var sCraft = skills("ui-premium-craft/SKILL.md");
var sMotion = skills("ui-motion-language/SKILL.md");
var sGraph = skills("node-graph-craft/SKILL.md");
ok(
  "three design skills ship (premium-craft, motion-language, node-graph-craft)",
  sCraft.length > 500 && sMotion.length > 500 && sGraph.length > 500,
  "missing skill body"
);
ok(
  "every skill carries the spec frontmatter (name + description)",
  /^---\r?\nname: [a-z0-9-]+\r?\ndescription: /.test(sCraft) && /^---\r?\nname: [a-z0-9-]+\r?\ndescription: /.test(sMotion) && /^---\r?\nname: [a-z0-9-]+\r?\ndescription: /.test(sGraph),
  "frontmatter malformed"
);
ok(
  "skills name their trigger conditions (what + when)",
  /[Ww]hen this skill applies/.test(sCraft) && /[Ww]hen this skill applies/.test(sMotion) && /[Ww]hen this skill applies/.test(sGraph),
  "no trigger section"
);
ok(
  "13.5.1 (review fix): the motion skill agrees with the implementation \u2014 layout motion is transform/opacity only, and short paint transitions (box-shadow, border-color, background, color, stroke) are explicitly allowed for visual state",
  /Layout motion uses only `transform` and `opacity`/.test(sMotion) && /Visual-state transitions may also animate paint/.test(sMotion) && !/Animate only `transform` and `opacity`/.test(sMotion),
  "motion rule still stricter than the CSS"
);
