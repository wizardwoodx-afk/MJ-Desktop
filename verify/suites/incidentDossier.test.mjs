...
function toSiemJsonl(d) {
  ...
  const lines = [{ kind: "dossier.head", ...head }];
...
var receipt = async (mission, outcome) => buildProofReceipt({
  mission,
  teamId: "team-alpha",
...
section("1. build \u2192 verify");
{
  ...
  ok("format is mj-incident-dossier/1", d.format === "mj-incident-dossier/1");
...
section("3. broken evidence is marked, never laundered");
{
  ...
  broken.events[1].data.outcome = "doctored";
  const d = await buildIncidentDossier({
    incidentId: "inc-3",
    mission: "m-broken",
    teamId: "t",
