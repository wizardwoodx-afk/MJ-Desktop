...
function materialise(nodes, wires, label) {
  ...
  wires.forEach(([sk, sp, tk, tp]) => {
    ...
    if (!src || !tgt) {
      dropped.push(`${label}: unknown key ${sk}->${tk}`);
      return;
    }
    ...
    const t = find(tgt, "inputs", tp);
    if (!s) {
      dropped.push(`${label}: no output port "${sp}" on ${src.title}`);
      return;
    }
    if (!t) {
      dropped.push(`${label}: no input port "${tp}" on ${tgt.title}`);
      return;
    }
    if (!portsCompatible(s.dataType, t.dataType)) {
      dropped.push(`${label}: ${src.title}.${s.id}(${s.dataType}) -> ${tgt.title}.${t.id}(${t.dataType}) type-incompatible`);
      return;
    }
...
for (const tpl of WORKFLOW_TEMPLATES) {
  ...
  let order = [];
  try {
    order = topoSort(instances, conns);
  } catch {
    order = [];
  }
