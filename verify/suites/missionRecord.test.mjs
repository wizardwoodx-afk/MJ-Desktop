...
async function buildMissionRecord(mission, opts) {
  ...
  const body = {
    ...
    // the crown artifact stamps the LIVE release — never a stale literal (14.1.2 review fix)
    cycles: recordCycles,
    economics,
...
async function verifyMissionRecord(r) {
  const reasons = [];
  if (r.format !== "mj-mission-record/1") reasons.push(`unknown format ${r.format}`);
...
describe("missionRecord \u2014 one signed file per mission", () => {
  it("\xA71 seals from real stores and verifies \u2014 economics identical to the chargeback row", async () => {
    ...
    assert.ok(r);
    assert.equal(r.format, "mj-mission-record/1");
    assert.equal(r.mjVersion, MJ_VERSION);
  ...
  it("\xA74 the standalone CLI verifies with zero MJ state", async () => {
    ...
    const mk = structuredClone(r);
    mk.issuer.publicKeyHex = "zz".repeat(32);
    const mkFile = path2.join(dir, "malformed.json");
    fs.writeFileSync(mkFile, JSON.stringify(mk));
    const mkRun = runCli(mkFile, ["--issuer-key", "1".repeat(64)]);
    assert.equal(mkRun.code, 1, mkRun.out);
    assert.match(mkRun.out, /malformed issuer/);
