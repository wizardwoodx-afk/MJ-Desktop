  ...
  for (const seat3 of team.seats) {
    if (!seat3.mayWrite) {
      if (opts.deferReview && hasWriter) {
        const path5 = `${root}-vh-review-${branchSafe(seat3.id)}`;
        plans.push({
          seatId: seat3.id,
          branch: "",
          path: path5,
    ...
    const path4 = `${root}-mj-${branchSafe(seat3.id)}`;
    plans.push({
      seatId: seat3.id,
      branch,
      path: path4,
...
function makeRepo() {
  const repo = fs.mkdtempSync(path3.join(os2.tmpdir(), "mjloop-"));
  fs.writeFileSync(path3.join(repo, "README.md"), "# guard\n\nAn authorization guard.\n");
  fs.writeFileSync(path3.join(repo, "guard.js"), BUGGY_GUARD);
  fs.writeFileSync(
    path3.join(repo, "test.js"),
...
`
  ...
  fs.writeFileSync(path3.join(repo, "package.json"), JSON.stringify({ name: "guard", version: "1.0.0", scripts: { test: "node test.js" } }, null, 2));
  sh(["git", "init", "-q", "."], repo);
