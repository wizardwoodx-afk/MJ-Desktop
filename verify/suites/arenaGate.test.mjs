...
async function runGovernanceArena(args = {}) {
  ...
  results.push(
    await scenario("arena.budget-race", "Two concurrent seats both try to reserve the last of the budget", async () => {
      ...
      const second2 = gate2.reserve("seat-b", 60);
      const held = first !== null && second2 === null && gate2.committedUsd === 60;
      return { held, note: held ? "second reservation refused; cap never crossed" : `first=${!!first} second=${!!second2} committed=${gate2.committedUsd}` };
    })
  );
  results.push(
