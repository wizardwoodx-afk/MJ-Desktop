...
test("identity strings are Vouch Harbor", () => {
  ...
  assert.ok(ver.includes('export const VH_VERSION = "16.5.0"'), "product version constant");
});
...
test("the offline CLI accepts both wires and rejects tampering", async () => {
  ...
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const fresh = await buildChainedReceipt2({ mission: "cli-fresh-check", teamId: "vouch", startedAt: now, finishedAt: now, version: "16.5.0", edition: "offline", events: [{ kind: "vouch.session", seatId: "vouch-core", data: { x: 1 } }] });
