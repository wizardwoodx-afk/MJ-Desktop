...
}
{
  ...
  ok(cline.env.CLINE_COMMAND_PERMISSIONS !== void 0, "cline gets command permissions via env, its only mechanism");
...
{
  for (const id of ["claude", "codex", "opencode", "grok", "cursor", "cline", "kilo"]) {
...
{
  for (const id of ["claude", "codex", "opencode", "grok", "cline"]) {
    const s = mkSeat(id);
