...
// src/graph/lint.ts
var nameOf = (g, id) => g.nodes.find((n) => n.id === id)?.title ?? id;
function lintGraph(g) {
  ...
  const seenWire = /* @__PURE__ */ new Set();
  for (const c of g.connections) {
    ...
    const sp = sn.outputs.find((p2) => p2.id === c.sourcePortId);
    const tp = tn.inputs.find((p2) => p2.id === c.targetPortId);
    if (!sp || !tp) {
    ...
    if (!portsCompatible(sp.dataType, tp.dataType)) {
      issues.push({ code: "TYPE_MISMATCH", severity: "error", nodeId: tn.id, message: `"${sn.title}\xB7${sp.label}" (${sp.dataType}) cannot feed "${tn.title}\xB7${tp.label}" (${tp.dataType}).` });
    }
  ...
  for (const n of g.nodes) {
    ...
    for (const p2 of n.inputs) {
      if (p2.required && !g.connections.some((c) => c.targetNodeId === n.id && c.targetPortId === p2.id)) {
        issues.push({ code: "REQUIRED_INPUT_OPEN", severity: "error", nodeId: n.id, message: `"${n.title}" needs input "${p2.label}" \u2014 it is unconnected.` });
      }
    }
...
== ${t} ==`);
var mkNode = (defId, id) => createNodeFromDef(DEFINITIONS_BY_ID.get(defId), id, 0, 0);
var mkGraph = (nodes, connections) => ({
...
var solo = mkNode("agent.coder", "a");
var single = lintGraph(mkGraph([solo], []));
...
var a = mkNode("agent.coder", "a");
var b = mkNode("agent.researcher", "b");
var ghost = mkGraph([a, b], [conn("c1", a, a.outputs[0].id, { ...b, id: "missing" }, b.inputs[0].id)]);
ok("a wire to a missing node is DANGLING_WIRE", lintGraph(ghost).some((i) => i.code === "DANGLING_WIRE" && i.severity === "error"), "");
var noPort = mkGraph([a, b], [conn("c1", a, "nope", b, b.inputs[0].id)]);
ok("a wire from a missing port is DANGLING_WIRE too", lintGraph(noPort).some((i) => i.code === "DANGLING_WIRE"), "");
var fakeSrc = { ...a, id: "src", outputs: [{ ...a.outputs[0], id: "o1", dataType: "Image" }] };
var fakeTgt = { ...b, id: "tgt", inputs: [{ ...b.inputs[0], id: "i1", dataType: "Boolean", required: false }] };
var mismatched = mkGraph([fakeSrc, fakeTgt], [conn("c1", fakeSrc, "o1", fakeTgt, "i1")]);
...
var dup = mkGraph([a, b], [conn("c1", a, a.outputs[0].id, b, b.inputs[0].id), conn("c2", a, a.outputs[0].id, b, b.inputs[0].id)]);
ok("the same wire twice is DUPLICATE_WIRE", lintGraph(dup).some((i) => i.code === "DUPLICATE_WIRE"), "");
var loop = mkGraph([a, b], [conn("c1", a, a.outputs[0].id, b, b.inputs[0].id), conn("c2", b, b.outputs[0].id, a, a.inputs[0].id)]);
var loopIssues = lintGraph(loop);
...
var openInput = mkGraph([b], []);
var reqOpen = lintGraph(openInput).filter((i) => i.code === "REQUIRED_INPUT_OPEN");
ok("an unconnected REQUIRED input is an error (matches the run gate)", b.inputs.some((p2) => p2.required) ? reqOpen.length > 0 && reqOpen[0].severity === "error" : true, JSON.stringify(reqOpen));
section("2. warnings \u2014 never refuse, always say");
var orphan = mkNode("control.wait", "c");
var start = mkNode("control.start", "s");
var planner = mkNode("agent.planner", "p");
var coder = mkNode("agent.coder", "k");
var legalChain = mkGraph(
  [start, planner, coder],
  [conn("c1", start, "payload", planner, "goal"), conn("c2", planner, "summary", coder, "task")]
);
var warnGraph = mkGraph([...legalChain.nodes, orphan], legalChain.connections);
ok("an unwired extra node is ORPHAN_NODE (warn)", lintGraph(warnGraph).some((i) => i.code === "ORPHAN_NODE" && i.severity === "warn"), "");
var v = lintVerdict(lintGraph(warnGraph));
...
var app = fs.readFileSync(path.join(root, "src", "App.tsx"), "utf8");
