...
var KEY2 = "mj.autonomy.v1";
var memory = null;
function loadAutonomy() {
  if (memory) return memory;
  try {
    const raw = JSON.parse(localStorage.getItem(KEY2) || "null");
    memory = {
...
var SESSION_KEY = "mj.teammate.session.v1";
var WORKSPACE_KEY = "mj.teammate.workspace.v1";
var BOT_NAME = "ROGUE";
...
var session = loadTeammateSession();
var listeners = /* @__PURE__ */ new Set();
function commit() {
  store.set(SESSION_KEY, JSON.stringify(session));
  for (const cb of listeners) cb();
}
function teammateSession() {
  return session;
}
function addTeammateFact(text) {
  const t = text.trim();
  if (!t) return;
...
function removeTeammateFact(id) {
  session = { ...session, facts: session.facts.filter((f) => f.id !== id) };
  commit();
}
function addTeammatePreference(text) {
  const t = text.trim();
  if (!t) return;
...
function removeTeammatePreference(id) {
...
function newTeammateThread(title) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
function setActiveTeammateThread(id) {
...
function teammateWorkspaceFiles() {
...
mj: ${VH_VERSION} \xB7 brain: simulated (offline)`;
}
var TEAMMATE_TOOLS = {
  calculator: {
    name: "calculator",
...
${p}`;
  ...
  workspace_list: {
    ...
    run: async () => {
      const fs2 = teammateWorkspaceFiles();
...
function simulateTeammateAction(action) {
...
function checkPrediction(action, sim, ok, output) {
  if (!ok) return false;
  if (action.kind === "dispatch") {
...
function requestTeammateApproval(action, detail) {
...
function resolveTeammateApproval(id, ok) {
...
function routeTeammate(plan, mode) {
  const reasons = [];
...
function proposeTeammateSkill(c) {
  const fail = (reason) => {
    session = {
      ...session,
  ...
  const known = c.tool === "dispatch_mission" || TEAMMATE_TOOLS[c.tool] !== void 0;
  ...
  if (existing) {
    session = { ...session, skills: session.skills.map((s) => s.name === c.name.trim() ? { ...s, version: s.version + 1, when: c.when.trim(), steps: c.steps, tool: c.tool, updatedAt: nowIso2() } : s) };
    commit();
...
function bumpTeammateSkillRun(skillId, ok) {
  session = {
    ...session,
...
function rateTeammateRun(receiptId, score, note) {
...
async function sendTeammateMessage(input) {
  ...
  updateActive((t) => ({ ...t, messages: [...t.messages, { id: msgId, role: "teammate", text: "", ts: nowIso2(), trace: [], streaming: true }] }));
...
WARNINGS: ${simNow.warnings.join("; ")}` : ""}`;
      ...
      if (ok) {
        if (action.kind === "tool") {
          ...
          patchMsg((m) => ({ ...m, trace: m.trace.map((s) => s.kind === "dispatch" && s.status === void 0 ? { ...s, status: "dispatched" } : s) }));
        }
      }
...
function verifyTeammateReceipt(id) {
...
function teammateReceiptJsonl(id) {
  const ref = session.receipts.find((r) => r.id === id);
  return ref ? receiptToJsonl(ref.receipt) : null;
}
function exportTeammateMemoryMarkdown() {
  const s = teammateSession();
  const lines = [
    `# ${s.botName} \u2014 memory export`,
    ``,
    `Exported ${(/* @__PURE__ */ new Date()).toISOString()} from this machine (MJ ${VH_VERSION}).`,
...
var pageSrc = read("src/pages/TeammatePage.tsx");
var moduleSrc = read("src/mission/teammate.ts");
