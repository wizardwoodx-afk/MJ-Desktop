...
function scrubEnv(env) {
  ...
  for (const [k, v2] of Object.entries(env)) {
    ...
    v2.length >= 40 && /^[A-Za-z0-9_\-.=+/]+$/.test(v2) || /^[A-Za-z0-9_-]{12,}\.[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{12,}/.test(v2);
    if (!shaped) out[k] = v2;
  }
  return out;
}
  ...
  if (detectHost() === "tauri") {
    try {
      ...
      return {
        ...
        setFullscreen: (v2) => w.setFullscreen(v2),
        setAlwaysOnTop: (v2) => w.setAlwaysOnTop(v2)
      };
    } catch {
    }
  }
...
var ArtifactStore = class {
  ...
  rollback(artifactId, recorder, actor, reason) {
    ...
    const target = this.versionsOf(a.lineageRoot).find((v2) => v2.version === a.rollbackTargetVersion);
  ...
  hydrate(artifacts) {
    ...
    for (const [k, v2] of this.lineage) {
      this.lineage.set(
        k,
        v2.sort((x, y) => (this.artifacts.get(x)?.version ?? 0) - (this.artifacts.get(y)?.version ?? 0))
      );
    }
  }
  export() {
...
var OrganizationRuntime = class {
  ...
  spawn(input) {
    ...
    this.recorder.record({
      ...
      data: {
        ...
        permissions: Object.entries(granted).filter(([, v2]) => v2).map(([k]) => k),
        denied,
  ...
  hydrate(state) {
    ...
    if (state.taskUpdatedAt) for (const [k, v2] of Object.entries(state.taskUpdatedAt)) this.taskUpdatedAt.set(k, v2);
  }
...
var OrganizationSupervisor = class {
   ...
   */
  diagnoseOrganization(signals, plan2) {
    ...
    if (implFailures.length >= 1) {
      const hasArchitect = plan2?.steps.some((s) => s.kind === "architecture") ?? false;
      const hasTest = plan2?.steps.some((s) => s.kind === "test") ?? false;
...
function firstObject(...vals) {
  for (const v2 of vals) if (v2 && typeof v2 === "object") return v2;
  return null;
}
...
function permissionPreamble(granted, policy) {
  const allowed = Object.entries(granted).filter(([, v2]) => v2).map(([k]) => k);
  const denied = Object.entries(granted).filter(([, v2]) => !v2).map(([k]) => k);
  return [
...
var AcpHarness = class _AcpHarness {
  ...
  async invoke(task) {
    ...
    try {
      ...
      const preamble = task.grantedPermissions ? Object.entries(task.grantedPermissions).filter(([, v2]) => v2).map(([k]) => `- may ${k}`).join("\n") : "- workspace-only; everything else is denied by default";
  ...
  stats(harness, scope) {
    ...
    return {
      ...
      byTaskKind: Object.fromEntries(
        [...kindAcc.entries()].map(([k, v2]) => [
          k,
          { runs: v2.runs, successRate: v2.runs ? v2.success / v2.runs : 0, medianLatencyMs: median(v2.ms) }
        ])
      ),
      byLanguage: Object.fromEntries(
        [...langAcc.entries()].map(([k, v2]) => [k, { runs: v2.runs, successRate: v2.runs ? v2.success / v2.runs : 0 }])
      )
    };
  }
...
function selectHarness(ctx, ledger) {
  ...
  for (const [k, v2] of top) rationale.push(`${k}: ${v2.toFixed(2)} (weight ${ARBITRATION_WEIGHTS[k] ?? 0})`);
...
var ResourceManager = class {
  ...
  spend(delta) {
    for (const [k, v2] of Object.entries(delta)) {
      const key = k;
      this.usage[key] = this.usage[key] + v2;
    }
  ...
  maySpend(estimatedCostUsd, liveAgents) {
    ...
    for (const [limit, value, ceiling] of checks) {
      if (ceiling > 0 && value > ceiling) {
        const v2 = { limit, value, ceiling, at: (/* @__PURE__ */ new Date()).toISOString() };
        this.violations.push(v2);
        return v2;
      }
    }
    return null;
...
function planMission(mission, req) {
  ...
  return { plan: plan2, signals, frameworkScores: scores };
}
...
Deliver the smallest increment that satisfies: ${this.mission.successCriteria[0] ?? this.mission.objective}. Mark the rest as not done.`;
  ...
  buildGraphFromPlan(plan2) {
    this.graph = graphFromSteps(this.mission, plan2.steps);
...
// probe/plan-probe.test.ts
var m = instantiateTemplate("tpl.software-development", { objective: "Build a production-ready SaaS billing feature in TypeScript", name: "p", workspace: "." });
...
var rt = new MissionRuntime(m, createServices(), { allowSimulated: true, installed: { "local-test": true } });
var plan = rt.prepare();
...
var g = graphFromSteps(m, plan.steps);
console.log("graph nodes:", g.nodes.length, "wires:", g.connections.length);
var v = validateWorkflow(g);
console.log("validation issues:", JSON.stringify(v, null, 1));
for (const n2 of g.nodes) console.log("  node", n2.id, n2.definitionId, "| purpose set:", n2.purpose.trim().length > 0);
