  ...
  "node_modules/@tauri-apps/api/image.js"() {
    ...
    Image = class _Image extends Resource {
      ...
      static async fromPath(path3) {
        return invoke2("plugin:image|from_path", { path: path3 }).then((rid) => new _Image(rid));
      }
...
async function pollMeta(changeId, timeoutMs = 15e3) {
  const t0 = Date.now();
  for (; ; ) {
...
describe("M6 protocol \u2014 meta_propose / meta_status / meta_revert over the wire", () => {
  ...
  const pollMetaStatus = async (changeId) => {
    for (let i = 0; i < 100; i++) {
  ...
  const pollMetaList = async (needle) => {
    for (let i = 0; i < 100; i++) {
