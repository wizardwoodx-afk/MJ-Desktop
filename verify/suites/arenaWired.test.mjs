...
async function runGovernanceArena(args = {}) {
  ...
  results.push(
    await scenario("arena.budget-race", "Two concurrent seats both try to reserve the last of the budget", async () => {
      ...
      const first = gate2.reserve("seat-a", 60);
      const second = gate2.reserve("seat-b", 60);
      const held = first !== null && second === null && gate2.committedUsd === 60;
      return { held, note: held ? "second reservation refused; cap never crossed" : `first=${!!first} second=${!!second} committed=${gate2.committedUsd}` };
    })
  );
  results.push(
...
function sessionIdKind(harness) {
  const rc2 = resolveCaps(harness);
  if (rc2.custom) return "cli-chosen";
  return rc2.caps.sessionStart?.argv ? "mj-chosen" : "cli-chosen";
}
...
`;
  ...
  for (const w of waves) {
    for (const a of w) {
      ...
      const rc2 = resolveCaps(a.seat.harness);
      const caps = rc2.caps;
      if (rc2.custom && !rc2.registered) {
...
async function verifyProofReceipt(rc2) {
  if (rc2.format !== "mj-proof-receipt/1" && rc2.format !== "mj-proof-receipt/2") return { ok: false, reason: "unknown format" };
  let prev = "0".repeat(64);
  for (const e of rc2.events) {
  ...
  const seal2 = await hmacHex(prev, VERIFY_SECRET);
  if (seal2 !== rc2.seal) return { ok: false, reason: "seal mismatch" };
  if (rc2.format === "mj-proof-receipt/2" && rc2.signature) {
    if (!rc2.issuer?.publicKeyHex) return { ok: false, reason: "receipt is signed but carries no issuer public key" };
    const ok2 = await verifyIssuerSignature(prev, rc2.signature, rc2.issuer.publicKeyHex);
    if (!ok2) return { ok: false, reason: `issuer signature verification FAILED for chain head ${prev}` };
  }
  return { ok: true, events: rc2.events.length };
}
...
function minimalRequest(repoRoot) {
...
var tmp = fs.mkdtempSync(path2.join(os.tmpdir(), "mj-arena-wired-"));
section("0. executeTeam invokes the arena before any seat runs (PASS path)");
var FIXED_NOW = 175e10;
var report;
try {
...
var standalone = await runGovernanceArena({ now: FIXED_NOW });
ok(
  "the mission-path digest equals the standalone battery digest at the same clock",
  report?.arena?.digest === standalone.digest,
  `mission=${report?.arena?.digest} standalone=${standalone.digest}`
);
ok(
  "the stamp records 11 scenarios, all defended",
  report?.arena?.total === 11 && report.arena.defended === 11 && report.arena.breached === 0,
...
var refused = await executeTeam(minimalRequest(tmp), { ...stubDeps(), arenaRunner: async () => refusedReport });
...
ok(
  "the report still records the REFUSED stamp and digest",
  refused.arena?.gate === "REFUSED" && refused.arena?.digest === "f".repeat(64),
  refused.arena ? `gate=${refused.arena.gate}` : "arena=null"
);
section("2. the arena digest rides the mission proof receipt");
var rc = await buildProofReceipt({
...
var event = rc.events?.find((e) => e.kind === "arena.gate");
ok("the sealed chain carries an arena.gate event", !!event, "no arena.gate event found");
ok(
  "the event carries the PASS gate and the mission digest",
  event?.data?.gate === "PASS" && event?.data?.digest === report.arena?.digest,
  event ? JSON.stringify(event.data).slice(0, 160) : "no event data"
);
ok(
  "the event carries the defended/total evidence",
  event?.data?.defended === 11 && event?.data?.total === 11,
...
var legacy = await buildProofReceipt({
...
var events = legacy.events ?? [];
ok(
  "no arena.gate event appears without the stamp",
  !events.some((e) => e.kind === "arena.gate"),
