...
function sessionIdKind(harness) {
  ...
  return rc2.caps.sessionStart?.argv ? "vh-chosen" : "cli-chosen";
}
...
async function verifyProofReceipt(rc2) {
  if (rc2.format !== "vh-proof-receipt/2" && rc2.format !== "mj-proof-receipt/2" && rc2.format !== "mj-proof-receipt/1") return { ok: false, reason: "unknown format" };
  let prev = "0".repeat(64);
  ...
  const sealSecret = SEAL_SECRET_BY_FORMAT[rc2.format] ?? VERIFY_SECRET;
  const seal2 = await hmacHex(prev, sealSecret);
  if (seal2 !== rc2.seal) return { ok: false, reason: "seal mismatch" };
  if ((rc2.format === "vh-proof-receipt/2" || rc2.format === "mj-proof-receipt/2") && rc2.signature) {
