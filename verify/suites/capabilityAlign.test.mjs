...
var queryLedger = /* @__PURE__ */ new Map();
function resetCapabilityQueryLedger() {
  queryLedger.clear();
}
function capabilityQueryCount(dataset, now) {
...
async function executeCapability(args) {
  ...
  if (spent.length >= policy.maxQueriesPerWindow) {
    return { result: null, reason: `refused \u2014 privacy budget exhausted: ${spent.length} queries already counted in this ${Math.round(policy.windowMs / 6e4)}-minute window; repeated aggregates can reconstruct rows, so the budget is hard` };
  }
  spent.push(now);
  ...
  const values = DEMO_COMPANY_DATA.rows.map((r) => Number(r[request.field])).filter((v) => Number.isFinite(v));
...
section("4. the Privacy Guard \u2014 aggregate-only is made privacy-honest (11.14.2)");
{
  ...
  ok(
    ...
    capabilityQueryCount(DEMO_COMPANY_DATA.dataset, NOW + 5) === 4
  );
  let exhausted = null;
