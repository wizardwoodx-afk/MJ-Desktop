...
var init_client = __esm({
  "src/ipc/client.ts"() {
    ...
    ipc = {
      ...
      llmChat: async (req2) => {
        if (useTauri()) return tauriInvoke("llm_chat", { req: req2 });
        const key = localDb.secretGet(req2.secret_ref);
        if (req2.provider === "ollama" || req2.base_url?.includes("11434")) {
          try {
            const r = await fetch(`${req2.base_url || "http://127.0.0.1:11434"}/api/chat`, {
              method: "POST",
              ...
              body: JSON.stringify({
                ...
                stream: false,
                messages: [
                  ...req2.system ? [{ role: "system", content: req2.system }] : [],
                  ...req2.messages
                ]
              })
            });
        ...
        if (!key) throw new Error(`secret not found: ${req2.secret_ref}`);
