...
async function runVouchToolCall(tool, args, opts = {}) {
  ...
  const runAction = async () => {
    ...
    try {
      return { ok: true, output: await t.run(args), mission: null };
    } catch (e) {
...
if (vault.SEAL !== "impossible-seal-0x9f3d") { bad++; console.error("FAIL: SEAL missing or wrong"); }
if (bad) process.exit(1);
process.stdout.write("all tests pass\\n");
`;
var DRILL_SCENARIOS = [
...
function drillCrew(sc) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  return {
    id: `team.drill-${sc.id}`,
    name: `Drill crew (${sc.id})`,
    description: `Deterministic drill crew for the ${sc.id} scenario \u2014 built-in seat (the swappable-brain seam)`,
    seats: [
      { id: "writer", role: "coder", harness: "opencode", model: null, mayWrite: true, timeoutSecs: 60, maxTurns: 2, instructions: `Drill[${sc.id}]: ${sc.writerTask}` },
      { id: "reviewer", role: "reviewer", harness: "opencode", model: null, mayWrite: false, timeoutSecs: 60, maxTurns: 2, instructions: `Drill[${sc.id}]: review the writer's change against the scenario invariants. Read-only.` }
    ],
    revision: 1,
    updatedAt: now
  };
}
function drillDeps(sc, verifyLog) {
  return {
    git: async (args, cwd) => {
...
async function runDrill(scenarioId) {
  ...
  try {
    ...
    try {
      ...
      const events = [
        ...
        { kind: "vouch.drill", seatId: "vouch-core", data: { scenario: sc.id, label: sc.label, status, loopStatus, verifiedSeats: outcome.verifiedSeats, seatCount: outcome.seatCount, cycleNo: outcome.cycleNo, testTail: testTail.slice(-160), missionId, engine: `built-in deterministic seat (labeled) + real git + the repo's own ${sc.testCommand.join(" ")}` } },
    ...
    const report = {
      ...
      cycleNo: outcome.cycleNo,
      runMs,
      engine: `built-in deterministic seat (the swappable-brain seam) + real git + real ${sc.testCommand.join(" ")} verification + real governance arena`,
...
function registerDrillTool() {
  ...
  VOUCH_TOOLS["run_drill"] = {
    ...
    run: async (a) => {
      const r = await runDrill(String(a.scenario ?? ""));
      return r.output;
    }
  };
}
