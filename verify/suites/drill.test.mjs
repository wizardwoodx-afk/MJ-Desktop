...
var init_image = __esm({
  "node_modules/@tauri-apps/api/image.js"() {
    ...
    Image = class _Image extends Resource {
       ...
       * ```
       */
      static async fromPath(path5) {
        return invoke2("plugin:image|from_path", { path: path5 }).then((rid) => new _Image(rid));
      }
...
var init_client2 = __esm({
  "src/ipc/client.ts"() {
    ...
    ipc2 = {
      ...
      fsRead: async (path5) => {
        if (useTauri()) return tauriInvoke("fs_read", { path: path5 });
        throw new Error("Filesystem is available in the native desktop build.");
      },
      fsWrite: async (path5, content) => {
        if (useTauri()) return tauriInvoke("fs_write", { path: path5, content });
        throw new Error("Filesystem is available in the native desktop build.");
      },
      fsList: async (path5) => {
        if (useTauri()) return tauriInvoke("fs_list", { path: path5 });
        return [];
      },
      fsMkdir: async (path5) => {
        if (useTauri()) return tauriInvoke("fs_mkdir", { path: path5 });
      },
      fsRemove: async (path5, recursive) => {
        if (useTauri()) return tauriInvoke("fs_remove", { path: path5, recursive });
      },
...
function parseStatusPorcelainZ(raw) {
  ...
  for (let i = 0; i < fields.length; i += 1) {
    ...
    out.push({ status, path: path5, oldPath });
  }
  return out;
}
  ...
  for (const seat2 of team.seats) {
    if (!seat2.mayWrite) {
      if (opts.deferReview && hasWriter) {
        const path6 = `${root}-vh-review-${branchSafe(seat2.id)}`;
        plans.push({
          seatId: seat2.id,
          branch: "",
          path: path6,
          createArgv: [],
          removeArgv: [["worktree", "remove", "--force", path6]],
          shared: false,
          deferred: true,
    ...
    const path5 = `${root}-mj-${branchSafe(seat2.id)}`;
    plans.push({
      seatId: seat2.id,
      branch,
      path: path5,
      createArgv: [["worktree", "add", "-b", branch, path5, opts.baseBranch]],
...
function reviewWorktreeArgv(snapshotBranch, path5) {
  return [["worktree", "add", "--detach", path5, snapshotBranch]];
}
...
var NodeAcpTransport = class {
  ...
  async start(onLine, onExit) {
    ...
    const child = spawn2(this.program, this.args, {
      cwd: this.cwd,
...
function getHarness(id) {
  ...
  if (typeof id === "string" && isCustomHarness(id)) {
    const { caps, registered: registered2 } = resolveCaps(id);
...
async function runVouchToolCall(tool, args, opts = {}) {
  ...
  const action = isDispatch ? { kind: "dispatch", objective: String(args.objective ?? "").trim() } : { kind: "tool", tool, args };
  const risky = RISKY_TOOLS.has(tool);
  ...
  if (action.kind === "dispatch" && action.objective.length === 0) {
...
WARNINGS: ${sim.warnings.join("; ")}` : ""}`;
  ...
  void approvalPromise.then((granted) => settle(granted)).then((result) => {
...
module.exports = { authorize };
`;
var GUARD_TEST = `const { authorize } = require("./guard");
let bad = 0;
...
`;
var MATHS_BASE = `function add(a, b) { return a + b; }
function multiply(a, b) { return a * b; }
module.exports = { add, multiply };
`;
var MATHS_FIXED = `function add(a, b) { return a + b; }
function multiply(a, b) { return a * b; }
...
`;
var MATHS_TEST = `const { add, multiply, clamp } = require("./maths");
let bad = 0;
...
process.stdout.write("all tests pass\\n");
`;
var VAULT_BASE = `module.exports = { OPEN: "open" };
`;
var VAULT_TEST = `const vault = require("./vault");
let bad = 0;
...
var DRILL_SCENARIOS = [
  {
    ...
    review: (repo) => {
      const src = fs.existsSync(path3.join(repo, "guard.js")) ? fs.readFileSync(path3.join(repo, "guard.js"), "utf8") : "";
      return src.includes("deny disabled admins") && !src.includes("return true; // BUG") ? "CORRECT: the guard now denies disabled admins (checked the real file on disk)." : "WRONG: guard.js does not deny disabled admins.";
    }
  },
  {
    id: "maths",
    ...
    review: (repo) => {
      const src = fs.existsSync(path3.join(repo, "maths.js")) ? fs.readFileSync(path3.join(repo, "maths.js"), "utf8") : "";
      return src.includes("function clamp") && src.includes("clamp") ? "CORRECT: clamp is implemented and exported (checked the real file on disk)." : "WRONG: clamp is missing from maths.js.";
    }
  },
  {
    id: "impossible",
    ...
    review: (repo) => {
      const src = fs.existsSync(path3.join(repo, "vault.js")) ? fs.readFileSync(path3.join(repo, "vault.js"), "utf8") : "";
      return src.includes("CLOSE") ? "NOTE: CLOSE was added, but the test also requires SEAL with a value outside the playbook \u2014 the test will fail. Reporting honestly." : "WRONG: no change landed in vault.js.";
    }
  }
];
function drillScenario(id) {
  return DRILL_SCENARIOS.find((s) => s.id === id) ?? null;
}
function drillCrew(sc, harness) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const seatHarness = harness ?? "opencode";
  const desc = harness ? `External-model drill crew for the ${sc.id} scenario \u2014 REAL seat: ${HARNESS_BY_ID.get(harness)?.name ?? harness}` : `Deterministic drill crew for the ${sc.id} scenario \u2014 built-in seat (the swappable-brain seam)`;
  return {
    id: `team.drill-${sc.id}`,
    name: `Drill crew (${sc.id})`,
    description: desc,
    seats: [
      { id: "writer", role: "coder", harness: seatHarness, model: null, mayWrite: true, timeoutSecs: 60, maxTurns: 2, instructions: `Drill[${sc.id}]: ${sc.writerTask}` },
      { id: "reviewer", role: "reviewer", harness: seatHarness, model: null, mayWrite: false, timeoutSecs: 60, maxTurns: 2, instructions: `Drill[${sc.id}]: review the writer's change against the scenario invariants. Read-only.` }
    ],
    revision: 1,
    updatedAt: now
  };
}
function drillDeps(sc, verifyLog, canonicalTest) {
  ...
  return {
    ...
    verify: async (cwd) => {
      if (canonicalTest && testFile && canary) {
        const p = path3.join(cwd, testFile);
      ...
      if (isWriter) {
        if (sc.fix) {
          for (const [rel, content] of Object.entries(sc.fix)) {
            fs.writeFileSync(path3.join(req.cwd, rel), content);
          }
        ...
        const vault = path3.join(req.cwd, "vault.js");
...
function drillReports() {
  return [...reports];
}
function newRepoId(prefix) {
  return `${prefix}${Date.now()}${Math.random().toString(36).slice(2, 6)}`;
}
function scenarioTestFile(sc) {
  return Object.keys(sc.files).find((f) => f.endsWith("test.js") || f.endsWith(".test.js")) ?? null;
}
function makeRepo(sc) {
  const repo = fs.mkdtempSync(path3.join(os2.tmpdir(), `vh-drill-${sc.id}-`));
  const testFile = scenarioTestFile(sc);
  ...
  for (const [rel, content] of Object.entries(sc.files)) {
    if (testFile && rel === testFile) {
      canonicalTest = content.replace("__SEAL__", seal2);
      continue;
    }
    const p = path3.join(repo, rel);
    fs.mkdirSync(path3.dirname(p), { recursive: true });
  ...
  if (canonicalTest) fs.writeFileSync(path3.join(repo, "canonical-test.js"), canonicalTest);
  return { repo, canonicalTest, seal: seal2 };
}
function testFileCanary(canonical) {
...
function whichBin(name) {
  ...
  for (const d of dirs) {
    for (const ext of exts) {
      const p = path3.join(d, name + ext.toLowerCase());
      try {
...
function findHarnessBin(harnessId) {
...
function realSeatDeps(sc, harnessId, verifyLog, canonicalTest) {
  const spec = HARNESS_BY_ID.get(harnessId);
  const base = drillDeps(sc, verifyLog, canonicalTest);
  return {
    ...base,
    resolveBin: async (bin) => spec.bins.includes(bin) ? whichBin(bin) : null,
    cliInvoke: async (req) => {
      const t0 = Date.now();
      try {
        const out = execFileSync(req.bin, req.argv, {
          cwd: req.cwd,
          encoding: "utf8",
          timeout: Math.max(15, req.timeoutSecs ?? 120) * 1e3,
...
async function runDrill(scenarioId, opts) {
  ...
  try {
    ...
    const loopStatus = outcome.status;
    const status = loopStatus === "completed" ? "passed" : loopStatus === "blocked" ? "blocked" : "failed";
    ...
    const report = {
      ...
      cycleNo: outcome.cycleNo,
      runMs,
      engine: realSeat ? `REAL seat: ${realSeat.name} (bin "${realSeat.bin}") \u2014 external-model validation + real git + real ${sc.testCommand.join(" ")} verification (canonical test, seat-invisible) + real governance arena` : `built-in deterministic seat (the swappable-brain seam) + real git + real ${sc.testCommand.join(" ")} verification (canonical test, seat-invisible) + real governance arena`,
    ...
    persistReports();
    const verdict = status === "passed" ? `DRILL ${sc.id}: PASSED \u2014 ${outcome.verifiedSeats}/${outcome.seatCount} seats verified, cycle ${outcome.cycleNo}, ${runMs}ms. The repository's own tests passed for real.` : `DRILL ${sc.id}: ${status.toUpperCase()} \u2014 the loop reported ${loopStatus}; ${outcome.verifiedSeats}/${outcome.seatCount} seats verified. ${status === "failed" ? "The scenario's own tests failed for real \u2014 reported honestly, nothing faked." : ""}`;
    const tail2 = testTail ? `
test output (tail): ${testTail}` : "";
    const output = `${verdict}
mission: ${missionId} \xB7 drill receipt: ${receiptId ?? "n/a"} \xB7 mission receipt: ${outcome.receiptOk ? "signed" : "missing"} \xB7 attestation digest: ${digest}${tail2}`;
...
function registerDrillTool() {
  ...
  VOUCH_TOOLS["run_drill"] = {
    ...
    run: async (a) => {
      const harness = typeof a.harness === "string" && a.harness.length > 0 ? a.harness : void 0;
      const r = await runDrill(String(a.scenario ?? ""), harness ? { harness } : void 0);
...
var forensics = null;
after(() => {
  if (forensics && fs2.existsSync(forensics)) fs2.rmSync(forensics, { recursive: true, force: true });
});
describe("drill \u2014 the product proving itself on real missions", () => {
  ...
  it("impossible: the honest-failure scenario \u2014 FAILED for real, never a fake pass, repo kept, still vouched", async () => {
    ...
    assert.ok(fs2.existsSync(r.report.repoPath), "the forensics repo is a real directory");
    const gitHead = fs2.existsSync(path4.join(r.report.repoPath, ".git"));
  ...
  it("the reproducible benchmark CLI (16.6.0): real run, honest verdicts, stable digest, cross-process attestation parity", async () => {
    ...
    const stdout = execFileSync3(process.execPath, ["tools/drill-benchmark.mjs"], {
      cwd: ROOT,
      encoding: "utf8",
      timeout: 24e4
    });
    ...
    const recomputed = crypto2.createHash("sha256").update(
      rep.scenarios.map((s) => [s.scenarioId, s.status, s.verifiedSeats, s.seatCount, s.cycleNo, s.engine, s.missionReceiptOk, s.testTail].join("|")).join("\n")
    ).digest("hex");
  ...
  it("benchmark integrity (16.7.0): two runs carry different SEALs, and the test file is out of every seat worktree", async () => {
    ...
    const firstSeal = fs2.readFileSync(path4.join(first.report.repoPath, "canonical-test.js"), "utf8").match(/SEAL !== "([0-9a-f]{32})"/)?.[1];
    assert.ok(firstSeal, "run 1 has a 128-bit SEAL in its canonical test");
    const second = await runDrill("impossible");
    const secondSeal = fs2.readFileSync(path4.join(second.report.repoPath, "canonical-test.js"), "utf8").match(/SEAL !== "([0-9a-f]{32})"/)?.[1];
    ...
    const tracked = execFileSync2("git", ["ls-files"], { cwd: first.report.repoPath, encoding: "utf8" }).split("\n").filter(Boolean);
  ...
  it("the external-model seam refuses honestly: unknown harness, missing CLI \u2014 never a silent fallback", async () => {
    const unknown = await runDrill("guard", { harness: "no_such_harness" });
