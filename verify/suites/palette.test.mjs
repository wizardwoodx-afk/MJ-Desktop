...
var useGraphStore = create((set, get) => {
  ...
  return {
    ...
    autoLayout: () => {
      const g = get().graph;
      const indeg = new Map(g.nodes.map((n2) => [n2.id, 0]));
      const adj = new Map(g.nodes.map((n2) => [n2.id, []]));
      ...
      const layers = [];
      let frontier = g.nodes.filter((n2) => (indeg.get(n2.id) ?? 0) === 0).map((n2) => n2.id);
      ...
      while (frontier.length) {
        ...
        frontier = next.filter((id, i, a2) => a2.indexOf(id) === i);
        if (layers.length > 40) break;
      }
      for (const n2 of g.nodes) if (!placed.has(n2.id)) layers.push([n2.id]);
