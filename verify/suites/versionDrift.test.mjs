...
var missionRecordSrc = read(path.join("src", "mission", "missionRecord.ts"));
ok(
  "missionRecord.ts imports MJ_VERSION (the record stamps the live release)",
  /from "..\/version"/.test(missionRecordSrc) && /mjVersion:\s*MJ_VERSION/.test(missionRecordSrc),
  "no MJ_VERSION import or use found"
);
ok(
  "no hardcoded release literal survives in missionRecord.ts",
...
for (const doc of docs) {
  ...
  ok(
    `${doc} title carries the exact release patch (${MJ_VERSION})`,
...
for (const doc of OPERATIONAL_DOCS) {
  let body = read(doc).replace(/docs\/history\/MJ-[0-9.]+[-A-Za-z0-9_]*\.md/g, "").replace(/\bMJ-[0-9]+\.[0-9]+\.[0-9]+-[A-Za-z0-9-]*\.md\b/g, "");
  const staleTokens = [...new Set(
    [...body.matchAll(/MJ[ _-]?(\d+\.\d+\.\d+)/g)].map((m) => m[1]).filter((v) => v !== MJ_VERSION)
  )];
  ok(
    `${doc} names no release other than ${MJ_VERSION} in its body`,
