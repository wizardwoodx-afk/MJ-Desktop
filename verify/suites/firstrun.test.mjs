...
// src/graph/store.ts
var THEME_IDS = ["ink", "pitch", "slag", "fern", "ivory", "travertine"];
var THEME_ALIASES = {
  obsidian: "ink",
  wabi: "ink",
  nth: "ink",
...
function getEditorPrefs() {
  try {
    ...
    if (raw) {
      ...
      const theme = THEME_IDS.includes(stored) ? stored : THEME_ALIASES[stored] ?? "ink";
      return {
...
var useGraphStore = create((set, get) => {
  ...
  return {
    ...
    restoreGraph: (g2, label) => withHistory(label, (cur) => {
      const vp = cur.viewport;
      Object.assign(cur, structuredClone(g2));
      cur.viewport = vp;
    }),
