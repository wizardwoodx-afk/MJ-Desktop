...
var riskPolicy_exports = {};
__export(riskPolicy_exports, {
...
function makeMission(objective) {
...
async function main() {
  ...
  const rt = new MissionRuntime(mission, services, {
    allowSimulated: true,
    // no coding CLI installed in this environment
  ...
  await criterion(5, "At least two different coding-agent harnesses can participate", async () => {
    ...
    ok(statsCodex.medianLatencyMs === 41e3 && statsClaude.medianLatencyMs === 54e3, "median latency wrong");
  ...
  await criterion(8, "Failure is classified", () => {
    ...
    const kinds = /* @__PURE__ */ new Set([...rt.getFailures().map((f) => f.kind), ...detected.map((e) => String(e.data.failureKind))]);
  ...
  await criterion(12, "Organization changes when necessary, under policy", () => {
    ...
    ok(
      mutations.length > 0 || reorgEvents.length > 0 || rt.getRepairs().some((r) => r.strategy === "SPAWN_SPECIALIST" || r.strategy === "REORGANIZE"),
  ...
  await criterion(20, "Mission history becomes reusable organizational memory", () => {
    ...
    ok(threw, "unscoped memory was accepted \u2014 \xA720 forbids dumping all memory into a context");
  ...
  await criterion(24, "Templates instantiate into valid DRAFT missions", () => {
    for (const t of MISSION_TEMPLATES) {
      ...
      ok(m.riskPolicy.autonomy !== void 0, `${t.id} has no autonomy mode`);
    }
  ...
  await criterion(25, "Risk classification gates CRITICAL actions to a human regardless of autonomy", async () => {
    const { classifyRisk: classifyRisk2, requiresHuman: requiresHuman2 } = await Promise.resolve().then(() => (init_riskPolicy(), riskPolicy_exports));
    eq(classifyRisk2("deploy the service to production").risk, "CRITICAL");
    eq(classifyRisk2("delete the production database").risk, "CRITICAL");
    eq(classifyRisk2("rotate the api key").risk, "CRITICAL");
    eq(classifyRisk2("git push --force origin main").risk, "CRITICAL");
    eq(classifyRisk2("npm install lodash").risk, "MEDIUM");
    eq(classifyRisk2("run the test suite").risk, "LOW");
    eq(classifyRisk2("do the thing").risk, "MEDIUM", "an unrecognised action must not default to LOW");
    ok(requiresHuman2("CRITICAL", "HIGH", "AUTONOMOUS"), "CRITICAL must always need a human");
    ok(!requiresHuman2("LOW", "HIGH", "AUTONOMOUS"), "LOW should be autonomous under AUTONOMOUS");
    ok(requiresHuman2("LOW", "LOW", "HUMAN_ONLY"), "HUMAN_ONLY must gate everything");
  });
