...
// probe/defs-probe.test.ts
var pick = ["agent.architect", "agent.coder", "agent.tester", "agent.reviewer", "agent.synthesizer", "agent.researcher"];
for (const id of pick) {
  ...
  console.log(id, "| in:", JSON.stringify((d.inputs ?? []).map((p2) => [p2.id, p2.label, p2.dataType, p2.required])), "| out:", JSON.stringify((d.outputs ?? []).map((p2) => [p2.id, p2.label, p2.dataType])));
}
