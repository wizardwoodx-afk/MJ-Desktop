...
async function requestEgress(args) {
  const { envelope, item: item2, recipient, now } = args;
...
var NOW = 176e10;
var item = { kind: "dossier", name: "test.json", sha256: "ab".repeat(32) };
section("1. refusal is the default, named in words");
{
  ...
  const env = await issueRootEnvelope({ principal: "human:runner", scope: ["egress:share"], expiresAt: NOW + 1e3 * 60 * 5, now: NOW });
  const expired = await requestEgress({ envelope: env, item, recipient: "r", now: NOW + 1e3 * 60 * 6 });
  ...
  const wrongScope = await issueRootEnvelope({ principal: "human:runner", scope: ["run:team-mission"], expiresAt: NOW + 1e3, now: NOW });
...
section("2. authorized departures land as receipts");
{
  ...
  ok(
    "the receipt names principal, recipient, item sha256 and envelope",
