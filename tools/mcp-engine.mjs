...
async function tauriInvoke(cmd, args) {
  const { invoke: invoke3 } = await import("@tauri-apps/api/core");
...
var AcpClient = class {
  ...
  async connect() {
    ...
    const agent = result2.agentInfo;
    return {
      protocolVersion: Number(result2.protocolVersion ?? PROTOCOL_VERSION),
  ...
  async newSession(cwd) {
    ...
    const result2 = await this.request("session/new", { cwd, mcpServers: [] });
    if (!result2?.sessionId) throw new Error("acp: session/new returned no sessionId");
    return result2.sessionId;
  }
  cancel(sessionId) {
  ...
  async prompt(sessionId, text) {
    ...
    try {
      const result2 = await this.request("session/prompt", {
        sessionId,
        prompt: [{ type: "text", text }]
      });
      return { stopReason: String(result2.stopReason ?? "end_turn"), text: chunks.join("") };
    } finally {
...
WARNINGS: ${sim.warnings.join("; ")}` : ""}`;
  ...
  void approvalPromise.then((granted) => settle2(granted)).then((result2) => {
...
var metaListeners = /* @__PURE__ */ new Set();
function persist() {
  if (HAS_LS) {
    try {
...
function metaChange(id) {
  return changes.find((c) => c.id === id) ?? null;
}
function currentGatedTools() {
  return [...RISKY_TOOLS].sort();
}
async function mintMetaReceipt(ev) {
  try {
...
function validateProposal(kindRaw, targetRaw, reasonRaw, isRevert) {
  const kind = kindRaw;
  const target = String(targetRaw ?? "").trim();
  const reason = String(reasonRaw ?? "").trim();
  const base = { ok: false, kind, target, from: "", to: "", simulation: { prediction: "", sideEffects: [], warnings: [] } };
  ...
  if (kind === "risk.tier") {
    ...
    const prediction2 = isRevert ? `Reverts the tightening of "${target}": it will run freely again (itself gated \u2014 this revert must be approved).` : `Tightens the gate on "${target}": every future call to it will SIMULATE first and pause at the HUMAN GATE before executing.`;
    return {
      ok: true,
      kind,
      target,
...
async function proposeMetaChange(kind, target, reason) {
  return runMetaFlow(kind, target, reason, false);
}
async function revertMetaChange(changeId) {
...
async function refuseImmediate(kind, target, words, reason) {
  const c = {
    id: newId("mc"),
    kind,
    target,
...
async function runMetaFlow(kind, targetRaw, reason, isRevert) {
...
WARNINGS: ${v.simulation.warnings.join("; ")}` : ""}`;
  ...
  void approvalPromise.then((approved) => settle(c, approved, v.simulation)).catch((e) => {
    c.status = "denied";
    c.decidedAt = nowIso3();
    c.decision = "denied";
    persist();
...
async function settle(c, approved, sim) {
  c.decidedAt = nowIso3();
  if (!approved) {
    c.status = "denied";
    c.decision = "denied";
    persist();
    const ref2 = await mintMetaReceipt({
      origin: "meta",
      changeId: c.id,
      verdict: "denied",
...
var PROTOCOL_VERSIONS = LEGACY_VERSIONS;
var TASKS_EXT = "io.modelcontextprotocol/tasks";
...
var SERVER_INFO = { name: "vouch-harbor", version: VH_VERSION };
var INSTRUCTIONS = "Vouch Harbor's governed capabilities. Every action tool call is routed through the product's pipeline (risk classification, human gate on risky calls, signed receipt per completed call). Modern clients (protocol 2026-07-28): gated calls return resultType 'input_required' (answer with inputResponses + requestState) or, when you declare the io.modelcontextprotocol/tasks extension, a durable task handle (poll tasks/get, answer tasks/update). Legacy clients: risky calls return pending with an approval id \u2014 use approve_action / deny_action, then poll call_status. Audit reads: mission_status, verify_receipt.";
var toolDef = (name, description, props, required = []) => ({
  name,
  description,
...
var TOOL_NAMES = new Set(MCP_TOOLS.map((t) => t.name));
var ACTION_TOOLS = /* @__PURE__ */ new Set(["calculator", "clock", "search", "web_search", "memory_save", "memory_recall", "preference_save", "workspace_list", "workspace_write", "dispatch_mission", "system_info", "run_drill"]);
function result(id, result2) {
  return JSON.stringify({ jsonrpc: "2.0", id, result: result2 });
}
function rpcError(id, code, message, data) {
  return JSON.stringify({ jsonrpc: "2.0", id, error: { code, message, ...data !== void 0 ? { data } : {} } });
}
...
function clientDeclaredTasks(caps) {
  const ext = caps?.extensions;
  return !!ext && typeof ext === "object" && TASKS_EXT in ext;
}
function modernResult(id, r) {
  const out = { ...r, resultType: r.resultType ?? "complete", _meta: { [META_SERVER_INFO]: SERVER_INFO, ...r._meta ?? {} } };
  return JSON.stringify({ jsonrpc: "2.0", id, result: out });
}
function modernText(id, text, isError = false) {
...
function approvalInputRequest(r, name) {
  return {
    human_approval: {
...
function decisionFromInputResponses(inputResponses) {
  if (!inputResponses || typeof inputResponses !== "object") return null;
  for (const v of Object.values(inputResponses)) {
    if (!v || typeof v !== "object") continue;
    const o = v;
    if (o.action === "accept") return true;
    if (o.action === "decline" || o.action === "cancel") return false;
    const content = o.content;
    if (content && typeof content === "object" && content.decision === "accept") return true;
    if (content && typeof content === "object" && content.decision === "decline") return false;
  }
  return null;
}
var tasks = /* @__PURE__ */ new Map();
var approvalToCall = /* @__PURE__ */ new Map();
function openTask(r, name, inputRequests) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const t = { taskId: `t_${r.callId}`, callId: r.callId, approvalId: r.approvalId ?? "", tool: name, createdAt: now, lastUpdatedAt: now, inputRequests };
...
async function governedTool(name, args) {
  ...
  if (name === "call_status") {
    ...
    const r2 = track.result;
    const receipt = r2.receiptId ? `
receipt: ${r2.receiptId}` : "";
    return { text: `call ${args.callId} [${track.tool}] done \u2014 ok=${r2.ok}, approved=${r2.approved}${receipt}
...
status: ${m.status} \xB7 cycle ${m.cycleNo} \xB7 verified ${m.verifiedSeats}/${m.seatCount} \xB7 gate ${m.gateStatus ?? "n/a"} \xB7 receipt ${m.receiptOk ? "signed" : "missing"}
crew: ${m.teamName} \xB7 ${m.runMs}ms \xB7 engine: ${m.engine}`,
      isError: false,
      pending: null
    };
  }
  ...
  if (name === "meta_propose") {
    const r2 = await proposeMetaChange(String(args.kind ?? ""), String(args.target ?? ""), String(args.reason ?? ""));
    const extra2 = r2.receiptId ? `
receipt: ${r2.receiptId}` : "";
    return { text: r2.output + extra2, isError: !r2.ok && !r2.pending, pending: null };
  }
  if (name === "meta_status") {
    ...
    if (want) {
      ...
      const rec = c.receiptId ? ` \xB7 decision receipt ${c.receiptId}` : "";
      const rv = c.revertedAt ? ` \xB7 REVERTED at ${c.revertedAt}${c.revertReceiptId ? ` (receipt ${c.revertReceiptId})` : ""}` : "";
      const ro = c.revertOf ? ` \xB7 revert of ${c.revertOf}` : "";
...
DECIDED: ${c.decision} at ${c.decidedAt}` : "\nDECIDED: pending at the human gate"}`, isError: false, pending: null };
    ...
    return {
      text: `Currently gated: ${currentGatedTools().join(", ")}.
` + all.slice(-10).map((c) => `${c.id} \u2014 ${c.kind} on "${c.target}" \u2014 ${c.status}${c.revertOf ? ` (revert of ${c.revertOf})` : ""}${c.receiptId ? ` [receipt ${c.receiptId}]` : ""}`).join("\n"),
      isError: false,
      pending: null
    };
  }
  if (name === "meta_revert") {
    const r2 = await revertMetaChange(String(args.changeId ?? ""));
    const extra2 = r2.receiptId ? `
receipt: ${r2.receiptId}` : "";
    return { text: r2.output + extra2, isError: !r2.ok && !r2.pending, pending: null };
  }
...
async function modernCallTool(name, args, params, id, caps) {
  try {
    if (params.inputResponses && typeof params.inputResponses === "object" && typeof params.requestState === "string") {
      ...
      return modernText(
        id,
        decision ? `Approved ${approvalId} \u2014 the call continues in the background; poll ${callRef} for the receipt.` : `Denied ${approvalId} \u2014 nothing was executed; the denial is recorded in the session.`,
        !decision
      );
    }
    const o = await governedTool(name, args);
...
function modernTasksGet(params, id, caps) {
...
function modernTasksUpdate(params, id, caps) {
  if (!clientDeclaredTasks(caps)) return missingTasksCapability(id);
  const t = tasks.get(String(params.taskId ?? ""));
  if (!t) return rpcError(id, -32602, `no such task "${String(params.taskId ?? "")}" on this server (tasks are per-process).`);
  const decision = decisionFromInputResponses(params.inputResponses);
...
function modernTasksCancel(params, id, caps) {
  if (!clientDeclaredTasks(caps)) return missingTasksCapability(id);
  const t = tasks.get(String(params.taskId ?? ""));
  if (!t) return rpcError(id, -32602, `no such task "${String(params.taskId ?? "")}" on this server (tasks are per-process).`);
  const track = vouchToolCallStatus(t.callId);
...
async function serveMcpStdio(readLine) {
  for (; ; ) {
