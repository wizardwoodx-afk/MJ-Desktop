...
receipt: ${r2.receiptId}` : "";
      return textResult(id, `call ${args.callId} [${track.tool}] done \u2014 ok=${r2.ok}, approved=${r2.approved}${receipt}
${r2.output}`, !r2.ok);
    }
...
crew: ${m.teamName} \xB7 ${m.runMs}ms \xB7 engine: ${m.engine}`
    ...
    if (name === "verify_receipt") {
      ...
      return v.ok ? textResult(id, `VALID \u2014 receipt ${args.receiptId} verifies (chain + seal${v.events ? ` + ${v.events} events` : ""}).`) : textResult(id, `INVALID \u2014 receipt "${String(args.receiptId ?? "")}": ${v.reason ?? "unknown"}.`, true);
    }
...
receipt: ${r2.receiptId}` : "";
      return textResult(id, r2.output + extra2, !r2.ok && !r2.pending);
    }
    if (name === "meta_status") {
  ...
  switch (msg.method) {
    case "initialize": {
      ...
      return result(id, {
        ...
        instructions: "Vouch Harbor's governed capabilities. Every action tool call is routed through the product's pipeline (risk classification, human gate on risky calls, signed receipt per completed call). Risky calls return pending with an approval id \u2014 use approve_action / deny_action, then poll call_status. Audit reads: mission_status, verify_receipt."
      });
    }
