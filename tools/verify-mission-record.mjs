...
const VERIFY_SECRET = "mj-commercial-v1-offline";
...
function verifyRecord(r) {
  ...
  if (signature) {
    ...
    else if (typeof issuer.publicKeyHex !== "string" || !/^[0-9a-fA-F]{64}$/.test(issuer.publicKeyHex)) {
      // 14.1.2 (external review): malformed keys get a clean verdict, never a stack trace.
      reasons.push("malformed issuer public key (expected 64 hex chars) — refusing to guess");
    } else {
      try {
        ...
        reasons.push("malformed issuer public key (unusable as an Ed25519 key)");
      }
    }
...
function main() {
  ...
  const recordedFp = /^[0-9a-fA-F]{64}$/.test(r.issuer?.publicKeyHex ?? "") ? keyFingerprint(r.issuer.publicKeyHex) : "(malformed)";
