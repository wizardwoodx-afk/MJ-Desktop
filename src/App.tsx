...
const NAV: Array<{ key: PageKind; icon: string; label: string }> = [
  ...
  { key: "control", icon: "radar", label: "Control" },
...
/**
 * 11.10.7 CLARITY — the rail was icon-only: thirteen glyphs, no words, no grouping.
 * Research (2026 sidebar UX practice) is unambiguous — icons may supplement labels,
 * never replace them, and related items belong under category headers. The sidebar
 * now reads in four groups; collapsed mode (Ctrl+B) drops to icon-only with tooltips.
 */
const NAV_GROUPS: Array<{ label: string; items: PageKind[] }> = [
  { label: "Workspace", items: ["home", "workflow", "missions"] },
  { label: "Fleet", items: ["teams", "control", "executions", "observability"] },
  { label: "Proof", items: ["proof", "evolution", "audit"] },
  { label: "System", items: ["mcp", "browser", "providers", "settings"] },
];
...
export function App() {
  ...
  const paletteActions: PaletteAction[] = useMemo(() => [
    ...
    { id: "home", label: "Go home", group: "Navigate", run: () => setPage("home") },
    ...
    { id: "assistant", label: "Open assistant", group: "Canvas", run: () => setAssistantOpen(true) },
  ...
  return (
    <div className={`app ${railCollapsed ? "rail-collapsed" : ""}`}>
      ...
      <aside className="rail sidebar" aria-label="Primary navigation">
        ...
        <button
          className="nav-assist"
          title="Assist · one custom node (Ctrl+J)"
          onClick={() => setAssistantOpen((o) => !o)}
        >
          {iconFor("spark")}
          <span className="nav-label">Assist</span>
        </button>
      </aside>
      ...
      <main className="main">
        ...
        <div className="stage" key={page}>
          ...
          {page === "control" && <ErrorBoundary label="control"><MissionControlPage /></ErrorBoundary>}
