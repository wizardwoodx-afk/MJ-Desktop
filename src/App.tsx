...
// Vouch Harbor 12.0 — routes are the FIVE doors of the one-product map (src/app/nav.ts):
...
/* VOUCH HARBOR 16.0 (the merge): the face door is now the Vouch control
 * plane — the accountable colleague, whose dispatch drives the real Mission
 * Loop. The 15.x internal "Teammate" prototype was retired from the nav in
 * 16.0 and DELETED in 16.6.0 (src/mission/teammate.ts, pages/TeammatePage.tsx,
 * probe/teammate.test.ts) — the one-throat rule (probe/vouch.test.ts) now pins
 * that no other file in src/ reaches the mission loop engine's entry point. */
const VouchPage = lazy(() => import("./vouch/pages/VouchPage").then((m) => ({ default: m.VouchPage })));
...
export function App() {
  ...
  const [page, setPage] = useState<PageKind>("teammate");
  ...
  useEffect(() => {
    ...
    window.addEventListener("vh://event", onEv);
    return () => window.removeEventListener("vh://event", onEv);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("vh:add-node", onAdd);
    return () => window.removeEventListener("vh:add-node", onAdd);
  }, [store]);
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (action) {
        ...
        case "screenshot": {
          ...
          if (!el) return;
          void snapshotCanvasToPng(el).then((png) => { downloadDataUrl(png, "vh-canvas"); toast("Screenshot saved"); });
          break;
        }
  ...
  const exportActive = async () => {
    ...
    downloadText(`${store.workflowName || "workflow"}.vouch.json`, JSON.stringify(pkg, null, 2));
    toast("Exported");
  };
  ...
  return (
    <div className={`app ${railCollapsed ? "rail-collapsed" : ""}`}>
      ...
      <header className="titlebar">
        ...
        <span className="logo">Vouch Harbor<i>.</i></span>
        <span className="host-pill">{host === "tauri" ? "native tauri" : "webview host"} · v{VH_SHORT} · {ROLE_PACK_COUNT} agents · {FRAMEWORK_COUNT} frameworks</span>
        {runningExecId && eta && (
      ...
      <div className="tabstrip">
        <button className={`tab ${page === "teammate" ? "active" : ""}`} onClick={() => setPage("teammate")}>Vouch</button>
      ...
      <main className="main">
        {/* V11.4 — keying the stage on the page re-mounts it on every view switch, which
            replays the 180ms vh-view-in fade (see vouch.css). Pages already mount lazily, so
            this adds the transition without changing what loads. */}
        <div className="stage" key={page}>
          <Suspense fallback={<div className="stage-fallback" style={{ color: "var(--text-mute, #6b6b6b)", fontFamily: "var(--font-mono, monospace)", fontSize: 12, letterSpacing: "0.08em", padding: 48 }}>LOADING — Vouch Harbor</div>}>
          {page === "teammate" && <ErrorBoundary label="vouch"><VouchPage /></ErrorBoundary>}
...
{page === "workflow" && (
      ...
      <footer className="statusbar">
        ...
        <span className="muted">Vouch Harbor {VH_SHORT}</span>
