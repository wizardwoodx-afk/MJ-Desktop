...
export function AuditPage() {
  ...
  const exportChargeback = async () => {
    ...
    await gatedDownload(`mj-chargeback-${new Date().toISOString().slice(0, 10)}.csv`, chargebackToCsv(cb), "file", setFinOpsNote);
  ...
  const exportMissionRecord = async () => {
    ...
    await gatedDownload(`mj-mission-record-${mission}-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify(r, null, 2), "dossier", setMrNote);
  ...
  const exportBlackBox = async () => {
    ...
    await gatedDownload(`mj-incident-${mission}-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify(d, null, 2), "dossier", setBbNote);
    ...
    <div className="panel-page">
      ...
      <div className="card">
        ...
        <p className="muted">
          ...
          tools/verify-receipt.mjs or verifyIncidentDossier(), no MJ required.
        </p>
      ...
      <div className="card">
        ...
        <p className="muted">
          ...
          Anyone can verify it with zero MJ state: <code>node tools/verify-mission-record.mjs record.json</code>
        </p>
        <div className="row" style={{ marginTop: 6 }}>
