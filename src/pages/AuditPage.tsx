...
export function AuditPage() {
  ...
  const exportDossier = async () => {
    const d = await buildProofDossier(Date.now());
    const blob = new Blob([JSON.stringify(d, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `mj-proof-dossier-${new Date(d.generatedAt).toISOString().slice(0, 10)}.json`;
    a.click();
    URL.revokeObjectURL(url);
    setDossierNote(`Exported. Digest ${d.digest.slice(0, 16)}… — re-hash the payload to verify; any edit breaks it.`);
  };
    ...
    <div className="panel-page">
      ...
      <div className="card">
        ...
        <p className="muted">
          ...
          re-hash the payload to confirm it was not edited after export.
        </p>
        <div className="row">
