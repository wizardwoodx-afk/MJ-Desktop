/**
 * MJ 12.0 — the MISSION LOOP page: the engine, one screen.
 *
 ...
 *
 * Every section on this page is a live view of the SAME loop (missionLoop.ts):
 * the crew panel is the team the loop runs; the channel feed is the loop's
 * communication; the cycle ledger is the loop's history with one signed
 * receipt per cycle; the decisions card is the human gate on the loop's
 * proposals; the learning strip is the loop's measured adaptation. There is
 * no second store and no second vocabulary — one engine, one screen.
 */
import { useEffect, useMemo, useState } from "react";
import { toast } from "../panels/Toast";
import { useTauri } from "../ipc/client";
import { PREBUILT_TEAMS, loadSavedTeams, saveTeams, upsertTeam } from "../mission/agentTeam";
import type { CliAgentTeam, TeamSeat } from "../mission/agentTeam";
import {
  runMissionLoopCycle,
  decideLoopCandidate,
...
import { hostRunnerDeps } from "../mission/hostDeps";
import { globalAgentBus, type InterAgentMessage } from "../mission/interAgentChannel";
...
export function LoopPage() {
  ...
  const [teams, setTeams] = useState<CliAgentTeam[]>(() => {
  ...
  const team = useMemo(() => teams.find((t) => t.id === teamId) ?? teams[0] ?? PREBUILT_TEAMS[0], [teams, teamId]);
  ...
  const [messages, setMessages] = useState<InterAgentMessage[]>(() => globalAgentBus.getMessages());
  const [channel, setChannel] = useState("#all");


  useEffect(() => subscribeMissionLoop(() => setLoopState(loadMissionLoopState())), []);
  useEffect(() => globalAgentBus.subscribe((m) => setMessages((prev) => [...prev.slice(-240), m])), []);
  useEffect(() => {
  ...
  const runCycle = async (count = 1) => {
    ...
    for (let i = 0; i < count; i++) {
      const res = await runMissionLoopCycle({
        ...
        mode,
        deps: hostRunnerDeps({ testCommand: testCmd }),
        emit: (ev) => {
  ...
  const decide = (c: TeamEvolutionCandidate, decision: "ACCEPTED" | "REJECTED") => {
    ...
    if (res.updatedTeam) {
      const next = upsertTeam(teams, { ...res.updatedTeam, updatedAt: new Date().toISOString() });
      saveTeams(next);
      setTeams(next);
  ...
  const saveCrew = () => {
    ...
    const next = upsertTeam(teams, updated);
    saveTeams(next);
    setTeams(next);
    ...
    <div className="panel-page" style={{ maxWidth: 1180 }}>
      ...
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p className="sub" style={{ marginTop: 0 }}>
            The engine: one crew, one cycle — dispatch, channels, gates, feedback, adaptation, receipts. MJ {MJ_VERSION_SHORT} · One Engine
      ...
      <div className="card" style={{ marginTop: 12 }}>
        ...
        <div className="muted" style={{ marginTop: 10, fontSize: 11 }}>
          One engine · one ledger · one signed receipt per cycle · human gate on every change · <span className="mono">MJ {MJ_VERSION_SHORT}</span>
        </div>
      </div>
    </div>
  );
