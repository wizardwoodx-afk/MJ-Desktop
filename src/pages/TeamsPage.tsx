...
export function TeamsPage({ onOpened }: { onOpened: () => void }) {
  ...
  const handleRunMission = async () => {
    ...
    try {
      ...
      // The adversarial gate judges this run on MEASURED facts (who wrote, who verified,
      // what the outcomes were), and the receipt it issues carries that verdict in-chain.
      let runGate: GateVerdict | null = null;
      try {
        runGate = gateForTeamReport(
          {
            status: res.status,
            seats: res.seats.map((s) => ({ seatId: s.seatId, role: s.role, harness: s.harness, outcome: s.outcome, verified: s.verified })),
            notRun: res.notRun.map((n) => n.seatId),
          },
          gatePolicy,
          true, // the receipt is issued below, in the same motion — gate and proof travel together
        );
        ...
        globalReceiptVault.issue({ mission: `mission-${runId}`, teamId: selectedTeam.id, gateStatus: runGate.status, gateTier: runGate.tier, receipt: rc });
                ...
                <button onClick={async () => {
                  ...
                  const rc = runnerReceipt ?? await buildProofReceipt({
                    ...
                    report: {
                      ...
                      seats: runnerResult.seats.map((s) => ({ seatId: s.seatId, role: s.role, outcome: s.outcome, verified: s.verified })),
