...
export function TeamsPage({ onOpened }: { onOpened: () => void }) {
  ...
  const handleExecuteMerge = async () => {
    ...
    try {
      ...
      const att = await buildMergeAttestation(result, MJ_VERSION);
  ...
  const handleRunMission = async () => {
    ...
    try {
      ...
      // 11.12 MOSAIC — the run travels with a signed Action Packet: intent, the belief
      // digest it rests on, the prediction (labelled, never evidence), the risk, the
      // human's permission (this button press), the rollback (isolated worktrees, no
      // auto-merge) and the pre-declared verification condition (the adversarial gate).
      // 11.12.1: the executor verifies it cryptographically before checking permission.
      ...
      const res = await executeTeam(
        {
          ...
          actionPacket, // 11.12 MOSAIC — proof-carrying action; verified + checked before spending
        },
        {
          ...deps,
          onTurn: (rec) => {
      ...
      // (whatever the CLI itself reported — MJ never estimates spend).
      try {
      ...
      // 11.12.1 (Varkha) — step-repetition escalation evidence, surfaced as SUGGEST only.
  ...
  return (
    <div className="panel-page animate-enter" style={{ maxWidth: 1200, margin: "0 auto" }}>
      ...
      {/* ═════════════════ 11.9.9 — ROLE BOARD: YOUR HARNESSES, YOUR ROLES ═════════════════
          MJ used to ship fixed vendor assignments. Not anymore: the user declares which
          ...
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 16, flexWrap: "wrap", marginBottom: 14 }}>
            <div>
              ...
              <div className="muted" style={{ maxWidth: 760, marginTop: 4 }}>
                ...
                role. Nothing here is fixed by MJ — User 1 can run Claude Code as coder, User 2 can run Grok Build as
            ...
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid var(--border)" }}>
                  ...
                  <div style={{ marginTop: 8, fontSize: 12, lineHeight: 1.6 }}>
                    ...
                    {runnerMergeAtt && (
                      <div className="row" style={{ marginTop: 8, alignItems: "center", gap: 8 }}>
                        ...
                        <button
                          onClick={() => downloadText(`mj-merge-attestation-${runnerRunId}.json`, JSON.stringify(runnerMergeAtt, null, 2), "application/json")}
                        >
                    ...
                    {runnerProvenance && (
                      <div className="row" style={{ marginTop: 8, alignItems: "center", gap: 8 }}>
                        ...
                        <button
                          onClick={() => downloadText(`mj-provenance-${runnerRunId}.json`, JSON.stringify(runnerProvenance, null, 2), "application/json")}
                        >
                          Download provenance
                        </button>
                        <button
              ...
              <div className="row" style={{ marginTop: 10 }}>
                <button onClick={async () => {
                  ...
                  downloadText(`mj-receipt-${runnerRunId}.jsonl`, receiptToJsonl(rc), "application/x-ndjson");
              ...
              <details style={{ marginTop: 8 }}>
                <summary className="muted" style={{ cursor: "pointer", fontSize: 11 }}>Verify a receipt (external check — no MJ state needed)</summary>
                <textarea value={verifyText} onChange={(e) => setVerifyText(e.target.value)} rows={4} style={{ width: "100%", marginTop: 6 }} placeholder="paste a mj-receipt .jsonl" />
