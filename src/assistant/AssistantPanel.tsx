    ...
    <div className="assistant-panel">
      <div className="asst-head">
        ...
        <div className="asst-title">
          ...
          <span className="asst-sub">designs one custom node · answers are labelled</span>
        </div>
        <span className={`asst-prov ${provLabel?.includes("offline") ? "offline" : ""}`} title="Who will answer — resolved before you type, labelled, never faked">
          {provLabel ?? "resolving…"}
        </span>
        <button className="ghost asst-close" onClick={onClose} title="Close (Esc)">✕</button>
      </div>

      <div className="msgs" ref={box}>
        {msgs.length <= 1 && (
          <div className="asst-welcome">
            <div className="asst-welcome-title">Describe one node. MJ builds exactly that.</div>
            <p className="muted" style={{ margin: "6px 0 12px", lineHeight: 1.55 }}>{INTRO}</p>
            <div className="asst-quick-label">Start from one of these</div>
            <div className="asst-suggestions">
              {QUICK_STARTS.map((q) => (
                <button key={q} className="asst-suggest" onClick={() => { setText(q); input.current?.focus(); }}>
                  {iconFor("spark")} {q}
                </button>
              ))}
            </div>
        ...
        {msgs.filter((m) => m.text !== INTRO || msgs.length === 1).map((m, i) => (
          <div key={i} className={`asst-msg ${m.role === "user" ? "user" : "bot"}`}>
            {m.role === "user" ? (
              <div className="asst-user-bubble">{m.text}</div>
            ) : (
              <div className="asst-bot-card">
                <div className="asst-bot-head">
                  <span className="asst-rail">
                    <span className="asst-dot" aria-hidden="true" />
                    <span className="asst-role">{m.via ? m.via.kind === "offline" ? "assist · offline draft" : "assist" : "assist"}</span>
                  </span>
                ...
                {m.insertedId && (
                  <div className="asst-actions">
                    ...
                    <button className="asst-action ghost" onClick={() => useGraphStore.getState().openDetails(m.insertedId!)}>
                      open inspector
                    </button>
                  </div>
                )}
      ...
      <div className="composer">
        <input
          ...
          disabled={busy}
        />
        <button className="primary asst-send" onClick={() => void send()} disabled={busy || text.trim().length === 0} title="Enter also sends">
          Design <span className="kbd">↵</span>
        </button>
      </div>
      <div className="asst-foot">local-first · one node per turn · offline drafts are labelled, never faked · Esc closes</div>
    </div>
  );
}
