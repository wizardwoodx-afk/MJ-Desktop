/**
 * §FIRST-RUN ONBOARDING (MJ 11.9.4-Commercial).
 *
 ...
 *   1. theme (wabi default — the five-color minimal flagship)
...
export function Onboarding({ onDone }: { onDone: () => void }) {
  const [theme, setTheme] = useState<"wabi" | "nth" | "inscribed">("wabi");
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 99, background: "var(--overlay, rgba(10,10,10,0.86))", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div className="card" style={{ maxWidth: 560, width: "92%", padding: 24, animation: "mj-node-in 0.34s cubic-bezier(0.2,0.9,0.25,1) both" }}>
        <div className="card-title" style={{ fontSize: 18 }}>Welcome to MJ</div>
        ...
        <div className="muted" style={{ marginTop: 14, fontSize: 12, textTransform: "uppercase", letterSpacing: "0.1em" }}>Pick a look (default: wabi)</div>
        <div className="row" style={{ marginTop: 6 }}>
          {(["wabi", "nth", "inscribed"] as const).map((t) => (
        ...
        <p className="muted" style={{ marginTop: 12, fontSize: 11 }}>
          The sample runs on MJ's labelled in-process harness — no API keys, no CLIs, nothing to install. Your first verified mission is one click away.
        </p>
      </div>
    </div>
  );
