...
export interface TeamRunRequest {
  ...
  /**
   * 11.12 MOSAIC — signed pre-action packet. 11.12.1: the executor verifies its
   * digest + signature BEFORE checking permission, BEFORE anything runs.
   */
...
export interface TeamRunReport {
  ...
  /** 11.12.1 (Varkha) — per-seat attenuated envelopes derived from the run's packet. */
  envelopes?: Array<{ seatId: string; scope: string[]; attenuatedFrom: string | null }>;
  /** 11.12.1 (Varkha) — step-repetition escalation signals observed in turn heartbeats. */
...
export async function executeTeam(req: TeamRunRequest, deps: TeamRunnerDeps, sessions = new SessionStore()): Promise<TeamRunReport> {
  ...
  // 11.12.1 (Varkha) — step-repetition signal: identical consecutive turn
  // signatures (MAST "step repetition") tracked in THIS scope via a wrapped
  // deps.onTurn; surfaced on the report as SUGGEST-level evidence only.
  ...
  let seatEnvelopes: Array<{ seatId: string; scope: string[]; attenuatedFrom: string | null }> = [];
  ...
  // 11.12.1 — proof-carrying actions, fully cryptographic at the boundary:
  ...
  // 11.12.1 (Varkha) — attenuation made explicit: each seat's envelope is derived
  // from (and never exceeds) the run's packet: role-scoped write/read rights and
  // ledger-capped spend, traced to the packet that permitted the run.
  seatEnvelopes = req.assignments.map((a) => ({
    seatId: a.seat.id,
    attenuatedFrom: req.actionPacket?.id ?? null,
    scope: [
      `role:${a.seat.role}`,
      a.seat.mayWrite ? `write:worktree:${wtBySeat.get(a.seat.id)?.branch ?? "unplanned"}` : "read:review-snapshot",
      "spend:capped-by-ledger",
    ],
  }));
