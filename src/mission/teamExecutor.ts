...
import { packetAllowsExecution, type ActionPacket } from "./actionPacket";
...
export interface TeamRunRequest {
  ...
  /**
   * 11.12 MOSAIC — signed pre-action packet. The executor refuses to start when
   * the packet is refused or an irreversible action lacks "allowed" permission.
   */
...
export interface TeamRunReport {
  ...
  strategy?: { id: string; gen: number; params: StrategyParams; isCandidate: boolean } | null;
  /** 11.12 MOSAIC — the action packet that permitted this run (outcome bound to intention). */
  actionPacket?: { id: string; digest: string; permission: string; reversible: boolean; verification: string } | null;
}
  ...
  const finish = (status: RunStatus, summary: string, spentUsd: number, snapshot: ReviewSnapshotRecord, briefings: BriefingRecord[]): TeamRunReport => {
    ...
    return {
      ...
      actionPacket: req.actionPacket
        ? { id: req.actionPacket.id, digest: req.actionPacket.digest, permission: req.actionPacket.permission, reversible: req.actionPacket.reversible, verification: req.actionPacket.verification }
        : null,
    };
  };

  // 11.12 MOSAIC — proof-carrying actions: the mechanical rule, checked BEFORE
  // anything executes or spends. Refused, or irreversible without "allowed",
  // aborts the mission with a plain-spoken summary.
