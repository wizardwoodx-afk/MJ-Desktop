 ...
 * Every candidate differs from its parent by EXACTLY one dimension, so each
 * experiment (including the mosaic one) is a paired toggle against the same
 * parent configuration; accumulated changes live in the parent, not the arm. */
