...
export function canonicalEnvelopeInput(e: Omit<AuthorityEnvelope, "digest" | "signature" | "signatureNote">): string {
  return JSON.stringify([e.format, e.id, e.principal, e.delegationChain, e.scope, e.issuedAt, e.expiresAt, e.revoked, e.parentId]);
}
...
export async function issueRootEnvelope(args: { principal: string; scope: string[]; expiresAt: number | null; now?: number }): Promise<AuthorityEnvelope> {
 ...
 */
export async function attenuate(parent: AuthorityEnvelope, agentId: string, subScope: string[], opts?: { expiresAt?: number | null; now?: number }): Promise<{ envelope: AuthorityEnvelope | null; reason: string }> {
