...
- 56 probe suites + 55 byte-pinned offline bundles, all green; mutation-tested (defects injected are caught; vacuous gates historically self-disclosed and fixed).
...
*Appendix — reproduce the diligence:* `npm ci && npm test` (56/56), `node verify/run.mjs` (55/55 offline), mutation-test any harness argv and watch the gate fail. The honesty is the moat.
