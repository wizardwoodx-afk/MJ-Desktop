...
- 53 probe suites + 52 byte-pinned offline bundles, all green; mutation-tested (defects injected are caught; vacuous gates historically self-disclosed and fixed).
...
*Appendix — reproduce the diligence:* `npm ci && npm test` (53/53), `node verify/run.mjs` (52/52 offline), mutation-test any harness argv and watch the gate fail. The honesty is the moat.
