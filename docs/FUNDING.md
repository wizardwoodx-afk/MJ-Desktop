...
- **Autonomy engines (2026 SOTA, wired into execution)** — bandit-routed strategy evolution (UCB1 + stagnation jump-arm, per RecHarness/AEL research), elastic seat scaling under human-on-the-loop supervision, live web evidence with primary/secondary source labelling.
...
- ~1 year of weekly cadence by a single founder (5.0 → 11.10), with a changelog that records its own defects (see CHANGELOG.md, e.g. the 11.7.0 and 11.9.1 entries).
...
| Soft license gate | Declared as a soft gate in-product; hard enforcement (signed native builds) on roadmap |
...
*Appendix — reproduce the diligence:* `npm ci && npm test` (59/59), `node verify/run.mjs` (58/58 offline), mutation-test any harness argv and watch the gate fail. Verification anyone can re-run is the moat.
