...
| Policy that is only documented gets bypassed | **Governance Arena Gate (11.14.7)** — a hostile battery attacks MJ's own authority machine (self-grading, agent root, scope growth, expiry, revocation, budget races, egress scope, tamper, ledger writes, off-policy data) before real runs; ANY breach refuses the mission in words, and the report's digest attaches to receipts | `src/mission/arenaGate.ts` | probe `arenaGate` (22) |
