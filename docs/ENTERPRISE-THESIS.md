...
The seed enforces the first four mechanically, today (MJ 11.14.2 — the
Privacy Guard: field policy, minimum cohort, hard per-window query budget,
bounded precision — each refusal in words, each pinned by probe).
Differential privacy is the enterprise tier, not the MVP.
