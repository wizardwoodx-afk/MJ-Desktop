...
This executes the 90 runtime probe suites as self-contained bundles — no `npm ci`,
no network, no `node_modules`. Expected tail:

```
OFFLINE VERIFY SUMMARY: 90 passed, 0 failed.
```
...
npm test              # 91 suites (90 runtime + the offline-pack freshness gate)
...
| Claim | Tier 1 | Tier 2 |
|---|---|---|
| The 90 runtime probe suites pass (incl. the 270-assertion harness suite) | ✔ | ✔ |
...
- **drill, mcpRouter, metaLoop, genome, mcpConformance, mcpSdkClient,
  mcpSdkClientV2** (16.2–16.9) — the drill + conformance line: real-mission
  drills with honest-failure scenarios, the MCP router (20 governed tools,
  byte-pinned engine bundle), the gated meta-loop, the capability genome
  (promotion line, quarantine, signed packages, auto-rollback), official
  schema conformance and both-era SDK-client validation.
  Measured on this tree: 91/91 live, 90/90 offline.
