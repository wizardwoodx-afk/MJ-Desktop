...
MJ ships its verification with it. There are two tiers — use the deepest one your
environment allows.
...
This executes the 59 runtime probe suites as self-contained bundles — no `npm ci`,
no network, no `node_modules`. Expected tail:

```
OFFLINE VERIFY SUMMARY: 59 passed, 0 failed.
```
...
npm test              # 60 suites (59 runtime + the offline-pack freshness gate)
...
| Claim | Tier 1 | Tier 2 |
|---|---|---|
| The 59 runtime probe suites pass (incl. the 270-assertion harness suite) | ✔ | ✔ |
