# BUILD MJ NATIVE — the exact steps (Windows 11)

MJ is a **Tauri v2 desktop application for Windows 11**. It is not a website and it does not need a server.
...
| Platform | Output |
|---|---|
| Windows | `src-tauri/target/release/bundle/nsis/MJ_13.5.1_x64-setup.exe` |
...
| `tauri build` → NSIS installer | ok — `MJ_13.5.1_x64-setup.exe`, installs per-user |
| Launch smoke | exe stays alive, main window titled "MJ", SQLite store created |
...
| Frontend has no dev-server dependency | the app branches on `detectHost()` and routes to IPC under Tauri |
| Mission/verification suites | 75/75 live, 74/74 offline |
