# Install MJ 12.0.0 on your laptop (Windows 11) — the only supported install
...
The one thing this environment cannot do is compile the Rust shell — there is no
toolchain here. Your `cargo check` is the first compile of this tree on a real
machine:
...
src-tauri\target\release\bundle\
  ├─ msi\MJ_12.0.0_x64_en-US.msi
  └─ nsis\MJ_12.0.0_x64-setup.exe
```
...
1. The header says **MJ 12.0** (Help → About must agree — `probe/versionDrift` enforces this).
