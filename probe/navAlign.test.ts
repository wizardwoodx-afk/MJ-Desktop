 ...
 *      loopHostDeps, loopBusFeed; knowledgeSkills — 12.1.0 the forge is
 *      engine API too) — a page that does is a fork of the engine.
...
/* 15.0.0 (Vouch Harbor): the sixth door — "teammate", the named human face
 * over the same engine. It leads the rail (the face is the front door), the
 * Mission Loop stays the engine. The group model is unchanged: Engine /
 * Verify / System; the engine group now holds teammate + loop + workflow. */
const EXPECTED_DOORS = ["teammate", "loop", "workflow", "proof", "audit", "settings"];

/* ── 0. root sanity (same idiom as versionDrift: never trust a broken root) ── */
const pkg = JSON.parse(read("package.json")) as { name?: string; version?: string };
ok("root resolves to the Vouch Harbor repo (package.json visible)", typeof pkg.name === "string" && /vouchharbor/i.test(pkg.name), `name=${String(pkg.name)}`);
...
ok("PageKind declares exactly the 15.0 doors (6)", kinds.length === 6 && kinds.join(",") === EXPECTED_DOORS.join(","), kinds.join(","));
const navKeys = [...navSrc.matchAll(/key: \"([a-z]+)\"/g)].map((m) => m[1]);
ok("the nav map lists each key exactly once", new Set(navKeys).size === 6 && navKeys.length === 6, navKeys.join(","));
...
ok("the rail leads with the Teammate door (the face), then the Mission Loop", navKeys[0] === "teammate" && navSrc.indexOf('key: "teammate"') < navSrc.indexOf('key: "loop"') && navSrc.indexOf('key: "loop"') < navSrc.indexOf('key: "workflow"'), navKeys[0]);
...
ok("engine holds teammate+loop+workflow, verify holds proof+audit, system holds settings",
  groupsSection.includes('n.key === "teammate" || n.key === "loop" || n.key === "workflow"') &&
...
ok("every nav entry carries a purpose description (6)", descriptions.length === 6, `${descriptions.length} descriptions`);
...
ok("the routes App can render are EXACTLY the six doors", routed.sort().join(",") === [...EXPECTED_DOORS].sort().join(","), routed.join(","));
...
ok("App opens on the Teammate door (the face, 15.0)", /useState<PageKind>\("teammate"\)/.test(appSrc));
...
ok("the Loop page imports none of the engine's internals either (crew store, host deps, bus — 12.0.1; knowledge forge — 12.1.0)",
...
const forgeSrc = read("src/mission/knowledgeSkills.ts");
const missionModules = [...new Set(loopImports.match(/from "\.\.\/mission\/[a-zA-Z]+/g) ?? [])];
ok("the engine module re-exports the knowledge-forge API (books → human-approved skills)",
...
ok("the Loop page reaches the forge through the engine only (ONE mission module imported)",
...
ok("the Loop page renders the Knowledge forge surface",
