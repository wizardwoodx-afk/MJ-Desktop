...
ok(`the advertised set is the fourteen-palette INSCRIBED set (${THEMES.length} themes)`, THEMES.length === 14, THEMES.join(","));
...
ok("nth carries the carbon ground (#0e0f0e)", nt("bg") === "#0e0f0e", nt("bg"));
ok("nth's signature accent is brass (#b8a17b)", nt("accent") === "#b8a17b", nt("accent"));
ok("nth's alive pulse is verdigris (#7ba58e)", nt("ok") === "#7ba58e", nt("ok"));
ok("nth's only brand-ish red is the functional oxide (#c0563f)", nt("danger") === "#c0563f", nt("danger"));
ok("nth selection inverts (brass fill, carbon text)", nt("sel-bg") === "#b8a17b" && nt("sel-fg") === "#0e0f0e", `${nt("sel-bg")} / ${nt("sel-fg")}`);
ok("nth active wires carry the brass signal", nt("wire-active") === "#b8a17b", nt("wire-active"));
...
/* ── §V11.9.4-Fixed: WABI — the twelfth palette, five-color minimal ────────
 * The 11.9.4(Fixed) rework made wabi the default: charcoal ground, washi ink,
 * one kiln-terracotta signal, moss alive, berry error — exactly five hues,
 * every secondary tone derived with color-mix so the count is structural.
 * Held here the same way nth is: exact hexes, luminance proof, no gradients,
 * and membership of every shared selector group so it is never a bolt-on.
 */
section("3.7. CYANOTYPE — the thirteenth palette, five-color rare (11.9.5)");
const cyanBlock = themesCss.match(/\[data-theme="cyanotype"\] \{[^}]*\n\}/)?.[0] ?? "";
const ct = (name: string): string => cyanBlock.match(new RegExp(`--${name}: (#[0-9a-fA-F]{6})`))?.[1] ?? "";
ok("the cyanotype token block exists", cyanBlock.length > 400, `block is ${cyanBlock.length} chars`);
ok("cyanotype ground is Prussian (#0b1524)", ct("bg") === "#0b1524", ct("bg"));
ok("cyanotype ink is paper (#e9eef7)", ct("text") === "#e9eef7", ct("text"));
ok("cyanotype signal is exposure cyan (#8fc6ff)", ct("accent") === "#8fc6ff", ct("accent"));
ok("cyanotype alive pulse is viridian (#5fb8a0)", ct("ok") === "#5fb8a0", ct("ok"));
ok("cyanotype error is madder (#d4687c)", ct("danger") === "#d4687c", ct("danger"));
ok("cyanotype derives its secondaries from the five (color-mix >= 20)", (cyanBlock.match(/color-mix\(/g) ?? []).length >= 20, `${(cyanBlock.match(/color-mix\(/g) ?? []).length} derivations`);
ok("cyanotype selection inverts (exposure fill, Prussian text)", ct("sel-bg") === "#8fc6ff" && ct("sel-fg") === "#0b1524", `${ct("sel-bg")} / ${ct("sel-fg")}`);
ok("cyanotype active wires carry the exposure signal", ct("wire-active") === "#8fc6ff", ct("wire-active"));
ok("cyanotype joins every shared selector group", (themesCss.match(/\[data-theme="cyanotype"\]/g) ?? []).length >= 118, `${(themesCss.match(/\[data-theme="cyanotype"\]/g) ?? []).length} refs`);

section("3.6. WABI — the twelfth palette, five-color minimal (default)");
const wabiBlock = themesCss.match(/\[data-theme="wabi"\] \{[^}]*\n\}/)?.[0] ?? "";
const wt = (name: string): string => wabiBlock.match(new RegExp(`--${name}: (#[0-9a-fA-F]{6})`))?.[1] ?? "";
...
/* ── §11.10.7: DAYLIGHT — the fourteenth palette, the clarity light look ───
 * Added with the CLARITY redesign: a warm-paper light theme so MJ reads clean
 * in bright rooms and on projectors. Held like the others: exact hexes,
 * luminance proof of lightness, five-hue discipline, no gradients.
 */
section("3.9. DAYLIGHT — the fourteenth palette, paper light (11.10.7)");
const dayBlock = themesCss.match(/\[data-theme="daylight"\] \{[^}]*\n\}/)?.[0] ?? "";
const dt = (name: string): string => dayBlock.match(new RegExp(`--${name}: (#[0-9a-fA-F]{6})`))?.[1] ?? "";
ok("the daylight token block exists", dayBlock.length > 400, `block is ${dayBlock.length} chars`);
ok("daylight ground is warm paper (#f6f3ed)", dt("bg") === "#f6f3ed", dt("bg"));
ok("daylight ink is near-black (#1c1915)", dt("text") === "#1c1915", dt("text"));
ok("daylight signal is kiln copper at light contrast (#b4492a)", dt("accent") === "#b4492a", dt("accent"));
ok("daylight alive pulse is moss (#3e7a5c)", dt("ok") === "#3e7a5c", dt("ok"));
ok("daylight error is madder (#b23a48)", dt("danger") === "#b23a48", dt("danger"));
ok("daylight derives its secondaries (color-mix >= 20)", (dayBlock.match(/color-mix\(/g) ?? []).length >= 20, `${(dayBlock.match(/color-mix\(/g) ?? []).length} derivations`);
ok("daylight is genuinely light (bg luminance > 0.7)", lum(dt("bg")) > 0.7, String(lum(dt("bg"))));
ok("daylight text is genuinely dark (luminance < 0.12)", lum(dt("text")) < 0.12, String(lum(dt("text"))));
ok("daylight uses no gradients (flat paper)", !/gradient\(/i.test(dayBlock), "gradient found");
ok("daylight joins the shared token group", /:is\([\s\S]*\[data-theme="daylight"\]/.test(themesCss), "not in the :is() group");
ok("store whitelist accepts daylight", storeSrc.includes('"daylight"'), "missing from THEME_IDS");
