...
test("the offline CLI accepts both wires and rejects tampering", async () => {
  ...
  const now = new Date().toISOString();
  const fresh = await buildChainedReceipt({ mission: "cli-fresh-check", teamId: "vouch", startedAt: now, finishedAt: now, version: "16.4.1", edition: "offline", events: [{ kind: "vouch.session", seatId: "vouch-core", data: { x: 1 } }] });
