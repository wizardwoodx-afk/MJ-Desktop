...
import { MJ_VERSION } from "../src/version";
...
describe("missionRecord — one signed file per mission", () => {
  it("§1 seals from real stores and verifies — economics identical to the chargeback row", async () => {
    ...
    assert.equal(r.mjVersion, MJ_VERSION); // stamps the LIVE release, never a stale literal (14.1.2 review fix)
  ...
  it("§4 the standalone CLI verifies with zero MJ state", async () => {
    ...
    const mk = structuredClone(r) as MissionRecord;
    mk.issuer!.publicKeyHex = "zz".repeat(32); // hostile input: garbage issuer key
    const mkFile = path.join(dir, "malformed.json");
    ...
    assert.match(mkRun.out, /malformed issuer/); // clean verdict, no stack trace (14.1.2)
