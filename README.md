# MJ 12.0.1 — the One-Engine desktop runtime for agent work
...
- **Explicit human feedback on every cycle** (12.0.1) — each cycle in the
  ...
  suppression, 3 is neutral). One engine over the engines of record — the
  model is one orchestrating runtime plus one cycle ledger, not a single
  physical store.
