...
probe/       # 47 suites (47/47)
verify/      # offline pack — 46 bundles + runner, byte-pinned
...
`npm test` does 47 suites — harnesses 270 assertions (the `iff` on turn-flags for all 25), checkRunner, sandbox, theme, canvasGeometry, etc. There's a typed `MjCommands` in `src/ipc/client.ts` so a renamed Rust command is a compile error, not a runtime surprise.
