...
- **Self-evolving organization** — measured runs reflect into a lessons memory that
  shapes future team briefings; a strategy archive adopts only *measured*
  improvement; skills learned from verified missions join briefings after human
  approval; and everything the org learns is signed as a learning receipt.
  Simulated runs teach it nothing about real execution.
...
node verify/run.mjs   # 59 bundles
```
...
probe/       60 probe suites, run by `npm test`
verify/      offline pack — 59 self-contained bundles + runner, byte-pinned
vendor/      reference MCP servers and the evolution service
docs/        verification notes, funding memo, per-release history
media/       screenshots and press assets
```
...
- Investor memo: [docs/FUNDING.md](docs/FUNDING.md)
